...
uses
  ...
  rp_BaseReport_unit, gd_security, prp_dlgCodeTemplates_unit, TB2ExtItems,
  StdCtrls;
...
type
  TfrmGedeminProperty = class(Tprp_frm)
    ...
    procedure actGetRUIDExecute(Sender: TObject);
  private
...
procedure TfrmGedeminProperty.actGetRUIDExecute(Sender: TObject);
var
  XID, DBID: TID;
begin
  if tbeRuidID.Text <> '' then
  begin
    if StrIsDigit(tbeRuidID.Text) then
    begin
      try
        tbeRuid.Text := gdcBaseManager.GetRUIDStringByID(StrToInt(tbeRuidID.Text));
      except
        tbeRuid.Text := 'Error!';
      end;
    end;
  end
  else if tbeRuid.Text <> '' then
  begin
    try
      if StrFind('_', tbeRuid.Text) > 0 then
      begin
        tbeRuidID.Text := IntToStr(gdcBaseManager.GetIDByRUIDString(tbeRuid.Text));
      end
      else
      begin
        XID := StrToInt(StrLeft(tbeRuid.Text, StrIPos(' ', tbeRuid.Text)));
        DBID := StrToInt(StrRestOf(tbeRuid.Text, StrIPos(' ', tbeRuid.Text) + 1));
        tbeRuidID.Text := IntToStr(gdcBaseManager.GetIDByRUID(XID, DBID));
      end
    except
      tbeRuidID.Text := 'Error!';
    end;
  end;
end;
