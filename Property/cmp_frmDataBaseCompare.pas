...
type
  TdcObjectType = (otUnknown, otProcedure, otTrigger, otView, otTable, otMacros, otFunction, otClass, otReport);
...
type
  TDataBaseCompare = class(TgdcCreateableForm)
    ...
    actAddPos: TAction;
    N1: TMenuItem;
    spcMain: TSuperPageControl;
    stbsConnection: TSuperTabSheet;
    stbsResult: TSuperTabSheet;
    pnBottom: TPanel;
    sbDBCompare: TStatusBar;
    spcResult: TSuperPageControl;
    stbsScripts: TSuperTabSheet;
    lvMacros: TgsListView;
    stbsMetadata: TSuperTabSheet;
    ...
    procedure actCompareSettingExecute(Sender: TObject);
    procedure actExtOpenExecute(Sender: TObject);
  private
    ...
    FList: TObjectList;
    FFileName: String;
    FScriptDiffCount, FMetadataDiffCount: Integer;
    ...
    procedure FillListViewDB(DS: TClientDataSet; ListView: TListView);
    procedure FillListViewSetting(DS: TClientDataSet; ListView: TListView);
...
uses
  ...
  at_dlgToSetting_unit, IBExtract, gdcStreamSaver, gsStreamHelper, gsMorph;

const
  SCRIPTS_CAPTION = 'Скрипты и макросы';
  METADATA_CAPTION = 'Метаданные';


procedure TDataBaseCompare.FormCreate(Sender: TObject);
begin
  spcMain.ActivePage := stbsConnection;
  spcResult.ActivePage := stbsScripts;
  ...
  // По умолчанию берем строку коннекта к серверу из текущего подключения
  if edExtServerName.Text = '' then
    edExtServerName.Text := IBLogin.ServerName;
...
begin
  ...
  try
    ...
    try
      ...
      if DSMacros.RecordCount > 0 then
        stbsScripts.Caption := SCRIPTS_CAPTION + ' (' + IntToStr(FScriptDiffCount) + ' ' +
          gsMorph.GetNumericWordForm(FScriptDiffCount, 'изменение', 'изменения', 'изменений') + ')';
      if DSMetaData.RecordCount > 0 then
        stbsMetadata.Caption := METADATA_CAPTION + ' (' + IntToStr(FMetadataDiffCount) + ' ' +
          gsMorph.GetNumericWordForm(FMetadataDiffCount, 'изменение', 'изменения', 'изменений') + ')';

      FillListViewDB(DSMacros, lvMacros);
      FillListViewDB(DSMetaData, lvMetaData);
  ...
  spcMain.ActivePage := stbsResult;
  Compared := True;
end;

procedure TDataBaseCompare.actCompareSettingExecute(Sender: TObject);
var
  FS: TFileStream;
...
begin
  ...
  try
    OD.DefaultExt := gsfExtension;
    OD.Filter := gsfxmlDialogFilter;
    OD.Title := 'Загрузить настройку из файла.';
    if OD.Execute then
    begin
      FNumber := 0;
      Compared := False;
      ...
      try
        ...
        begin
          ...
          lvMacros.Items.Clear;
          try
            FillListViewSetting(DSMacros, lvMacros);
          finally
            lvMacros.Items.EndUpdate;
          end;
          Compared := True;
          ...
          // Перейдем на таб с результатами
          spcMain.ActivePage := stbsResult;
        end
        else
          MessageBox(0,
...
begin
  ...
  try
    ...
    try
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              else if AnsiCompareStr(FSQL.FieldByName('FUNCTIONNAME').AsString, FExtSQL.FieldByName('FUNCTIONNAME').AsString) > 0 then
              begin
                //только на второй базе
                DS.Insert;
                if FMethod then
      ...
      else if (FSQL.RecordCount > 0) then
      begin
        while not FSQL.Eof do
        begin
          DS.Insert;
...
procedure TDataBaseCompare.FillListViewDB(DS: TClientDataSet; ListView: TListView);
          ...
          cn_just_first:
            begin
              ...
              begin
                ...
                ListItem.Caption := DS.FieldByName('FUNCTIONNAME').AsString + ' (' + DS.FieldByName('NAME').AsString + ')';
                ListItem.SubItems.Add(DS.FieldByName('EDITIONDATE1').AsString);
                ListItem.SubItems.Add('');
                ListItem.SubItems.Add('');
                ListItem.SubItems.Add('');
                ListItem.SubItems.Add('');
                ListItem.SubItemImages[1] := 156;
              end;
            end;
          cn_just_second:
            begin
              ...
              begin
                ...
                ListItem.SubItems.Add('');
                ListItem.SubItems.Add('');
                ListItem.SubItems.Add(DS.FieldByName('FUNCTIONNAME').AsString + ' (' + DS.FieldByName('NAME').AsString + ')');
                ListItem.SubItems.Add(DS.FieldByName('EDITIONDATE2').AsString);
                ListItem.SubItemImages[1] := 157;
              end;
            end;
          cn_eqv:
            begin
            ...
            begin
              ...
              begin
                ...
                if DS.FieldByName('RESULT').AsInteger = cn_first then
                  ListItem.SubItemImages[1] := 175
                else if DS.FieldByName('RESULT').AsInteger = cn_second then
                  ListItem.SubItemImages[1] := 189;
              end;
            end;
        end;
      end;
...
procedure TDataBaseCompare.FillListViewSetting(DS: TClientDataSet;
  ListView: TListView);
...
var
  ...
  IntEditionDate, ExtEditionDate: TDate;

  IBExtract, ExtIBExtract: TIBExtract;
begin
  FTransaction := TIBTransaction.Create(nil);
  FTransaction.Params.Add('read_committed');
  FTransaction.Params.Add('rec_version');
  ...
  try
    ...
    IBExtract.Database := gdcBaseManager.Database;
    IBExtract.Transaction := gdcBaseManager.ReadTransaction;
    ExtIBExtract := TIBExtract.Create(Self);
    ExtIBExtract.Database := ibExtDataBase;
    ExtIBExtract.Transaction := FTransaction;
    try
      if (FSQL.RecordCount > 0) and (FExtSQL.RecordCount > 0) then
      begin
        while (not FSQL.Eof) or (not FExtSQL.Eof) do
        begin
          ...
          begin
            ...
            begin
              IntEditionDate := FSQL.FieldByName('EDITIONDATE').AsDateTime;
              ExtEditionDate := FExtSQL.FieldByName('EDITIONDATE').AsDateTime;

              if MetadataType = cn_table then
              begin
                IBExtract.ExtractObject(
                  eoTable, FSQL.FieldByName('RELATION_NAME').AsString,
                  [etDomain, etTable, etForeign, etIndex, etCheck]);
                ProcText := IBExtract.Items.Text;

                ExtIBExtract.ExtractObject(
                  eoTable, FSQL.FieldByName('RELATION_NAME').AsString,
                  [etDomain, etTable, etForeign, etIndex, etCheck]);
                ExtProcText := ExtIBExtract.Items.Text;

                IntEditionDate := GetTableEditionDate(FSQL.FieldByName('RELATION_NAME').AsString, gdcBaseManager.Database);
                ExtEditionDate := GetTableEditionDate(FSQL.FieldByName('RELATION_NAME').AsString, ibExtDataBase);
              end
              else if MetadataType = cn_procedure then
              begin
              ...
              DS.FieldByName('EDITIONDATE1').AsDateTime := IntEditionDate;
              DS.FieldByName('EDITIONDATE2').AsDateTime := ExtEditionDate;
              if CompareStr(ProcText, ExtProcText) <> 0 then
              begin
                if IntEditionDate > ExtEditionDate then
                  DS.FieldByName('RESULT').AsInteger := cn_first
                else
                  DS.FieldByName('RESULT').AsInteger := cn_second;
                Inc(FMetadataDiffCount);
              end
              else
                DS.FieldByName('RESULT').AsInteger := cn_eqv;
              DS.FieldByName('ID').AsInteger := FSQL.FieldByName('ID').AsInteger;
            ...
            else if AnsiCompareStr(FSQL.FieldByName('RELATION_NAME').AsString, FExtSQL.FieldByName('RELATION_NAME').AsString) > 0 then
            begin
              //только на второй базе
...
begin
  ...
  begin
      ...
      begin
        ...
        try
          case TRecordData(lvMetaData.Selected.Data).FRecordInfo.Result of
            cn_eqv, cn_first, cn_second:
            begin
              if DSMetadata.Locate('ID', TRecordData(lvMetaData.Selected.Data).FRecordInfo.ID, []) then
              begin
                RelationName := DSMetadata.FieldByName('FUNCTIONNAME').AsString;

                IBExtract.Database := gdcBaseManager.Database;
                IBExtract.Transaction := gdcBaseManager.ReadTransaction;
                IBExtract.ExtractObject(
                  eoTable, RelationName,
                  [etDomain, etTable, etForeign, etIndex, etCheck]);
                IntSource := IBExtract.Items.Text;

                IBExtract.Database := ibExtDataBase;
                IBExtract.Transaction := FTransaction;
                IBExtract.ExtractObject(
                  eoTable, RelationName,
                  [etDomain, etTable, etForeign, etIndex, etCheck]);
                ExtSource := IBExtract.Items.Text;
              end;
            end;
            ...
            cn_just_second:
            begin
              if DSMetadata.Locate('ExtID', TRecordData(lvMetaData.Selected.Data).FRecordInfo.ExtID, []) then
              begin
                RelationName := DSMetadata.FieldByName('FUNCTIONNAME').AsString;
        ...
        case TRecordData(lvMetaData.Selected.Data).FRecordInfo.Result of
          ...
          begin
            ...
            try
              ...
              if Module = cn_procedure then
              begin
                IntSource := 'CREATE PROCEDURE "' + FSQL.FieldByName('RELATION_NAME').AsTrimString + '" ' +
                  GetParamsText(FSQL.FieldByName('RELATION_NAME').AsString, gdcBaseManager.Database) +  ' AS ' +
                  FSQL.FieldByName('RDB$PROCEDURE_SOURCE').AsTrimString;

                ExtSource := 'CREATE PROCEDURE "' + FExtSQL.FieldByName('RELATION_NAME').AsTrimString + '" ' +
                  GetParamsText(FExtSQL.FieldByName('RELATION_NAME').AsString, ibExtDataBase) +  ' AS ' +
                  FExtSQL.FieldByName('RDB$PROCEDURE_SOURCE').AsTrimString;
              end
              else if Module = cn_trigger then
              begin
                IntSource := 'CREATE TRIGGER "' + FSQL.FieldByName('RELATION_NAME').AsTrimString + '" ' +
                  FSQL.FieldByName('RDB$TRIGGER_SOURCE').AsTrimString;

                ExtSource := 'CREATE TRIGGER "' + FExtSQL.FieldByName('RELATION_NAME').AsTrimString + '" ' +
                  FExtSQL.FieldByName('RDB$TRIGGER_SOURCE').AsTrimString;
              end
              else if Module = cn_view then
              begin
                IntSource := GetViewTextBySource(FSQL.FieldByName('RDB$VIEW_SOURCE').AsTrimString,
                  FSQL.FieldByName('RELATION_NAME').AsTrimString);
                ExtSource := GetViewTextBySource(FExtSQL.FieldByName('RDB$VIEW_SOURCE').AsTrimString,
                  FExtSQL.FieldByName('RELATION_NAME').AsTrimString);
              end;
    ...
    ScriptComparer := Tprp_ScriptComparer.Create(nil);
    try
      ScriptComparer.Compare(IntSource, ExtSource);
      ScriptComparer.LeftCaption(DBName);
      ScriptComparer.RightCaption(ExtDBName);
      
      ScriptComparer.WindowState := wsMaximized;
      ScriptComparer.ShowModal;
    finally
      ScriptComparer.Free;
    end;
...
procedure TDataBaseCompare.actExtOpenExecute(Sender: TObject);
begin
