...
uses
  ...
  StdCtrls, ExtCtrls, Menus, Buttons, ImgList;
...
begin
  ...
  //!!!
  if (FImageList <> nil) and (ImageIndex > - 1) then
  begin
    X := PaintRect.Left + ((PaintRect.Right - PaintRect.Left - ImageList.Width) div 2);
    Y := PaintRect.Top + ((PaintRect.Bottom - PaintRect.Top - ImageList.Height) div 2); 
    ImageList.Draw(CacheCanvas, X, Y, ImageIndex, Enabled);
  end else
  //!!!
...
procedure TfrSpeedButton.SetImageIndex(const Value: TImageIndex);
begin
  FImageIndex := Value;
  Invalidate;
end;

procedure TfrSpeedButton.SetImageList(const Value: TImageList);
begin
  FImageList := Value;
  Invalidate;
end;
...
procedure TfrComboEdit.SetImageIndex(const Value: TImageIndex);
begin
  FButton.ImageIndex := Value;
end;

procedure TfrComboEdit.SetImageList(const Value: TImageList);
begin
  FButton.ImageList := Value;
end;

function TfrComboEdit.GetImageIndex: TImageIndex;
begin
  Result := FButton.ImageIndex;
end;

function TfrComboEdit.GetImageList: TImageList;
begin
