...
procedure TFileList.SomeListChanged(Sender: TObject);
begin
  DirChanged := true;
end;

function TFilelist.GetListIsfresh: Boolean;
begin
  Result := not DirChanged;
end;

procedure TFileList.SetListIsfresh(State: Boolean);
begin
  DirChanged := not State;
end;

procedure TFileList.AddFile(FileName: string; MainDirLen: Integer);
var
  CanAdd: Boolean;
  ShortName: string;
begin
...
function TFileList.CompareFiles(s1, s2: string): integer;
var
  cmp: Integer;
begin
  Result := 0;
...
procedure TFileList.QuickSort(List: TStringList; L, R: Integer);
var
  I, J: Integer;
  Average: string;
begin
...
procedure TFileList.ReadFiles(Dir: string; Mask: string;
                              MainDirlen: Integer);
var
  Status: Integer;
...
procedure TFileList.ReadDirectory(Dir: string; Mask: string;
                                  MainDirLen: integer);
var
  Status: Integer;
  TempName: string;
...
procedure TFileList.CreateList;
var
  i, j: Integer;
  ExcFull: TStringlist;
...
procedure TFileList.ListIsRead;
begin
...
procedure TFileList.SetActive(Value: Boolean);
begin
  FActive := Value;
  if DirChanged and Active then CreateList;
end;

function TFileList.GetNames(Index: integer): String;
begin
  if DirChanged and Active then CreateList;
  GetNames:=Items.Strings[index];
end;

function TFileList.GetFullNames(Index: integer): String;
begin
  if DirChanged and Active then CreateList;
  GetFullNames:=FullItems.Strings[index];
end;

function TFileList.GetList: TStringList;
begin
  if DirChanged and Active then CreateList;
  GetList:=Items;
end;

function TFileList.GetFullNamesList: TStringList;
begin
  if DirChanged and Active then CreateList;
  GetFullNamesList := FullItems;
end;

procedure TFileList.SetAnyList(AList: TStringList);
begin
...
function TFileList.ExpandInclude:TStringList;
var
...
function TFileList.ExpandExclude(InclList: TStringList):TStringList;
var
  SwapList: TStringList;
  i, j: Integer;
  Attr: Integer;
...
procedure TFileList.SetInclude(List: tStringlist);
begin
  FInclude.Assign(List);
  DirChanged := true;
end;

procedure TFileList.SetExclude(List: tStringlist);
begin
  FExclude.Assign(List);
  DirChanged := true;
end;

procedure TFileList.SetSorting(ASort: TSorting);
begin
...
procedure TFileList.SetTextCase(ACase: TTextCases);
begin
...
procedure TFileList.SetGoSubDirs(AGo: Boolean);
begin
...
procedure TFileList.CalcFilesSize;
var
  i: integer;
begin
  FFilesSize := 0;
...
function TFileList.GetFilesSize: LongInt;
begin
  if DirChanged and Active then CreateList;
...
procedure TFileList.Open;
begin
  Active := true;
end;

procedure TFileList.Close;
begin
  Active := false;
end;
...
procedure Register;
begin
  RegisterComponents('xStorm', [TFileList]);
end;
