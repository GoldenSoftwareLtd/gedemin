...
type
  Tgdc_frmSetting = class(Tgdc_frmMDHGR)
    ...
    procedure actCopyToSettingUpdate(Sender: TObject);
    procedure actMoveToSettingUpdate(Sender: TObject);
  ...
  private
    ...
    function CopySettingPos: Boolean;
...
uses
  ...
  {$ENDIF}
  , gdc_frmStreamSaver, gsStreamHelper, ShellAPI, prm_ParamFunctions_unit,
  flt_ScriptInterface, at_dlgToSetting_unit;
...
begin
  ...
  begin
    MessageBox(Handle,
      'Загрузка настройки возможна только под учетной записью Administrator.',
      'Отсутствуют права',
      MB_OK or MB_ICONHAND);
    exit;
  end;}
...
begin
  if CopySettingPos and actDetailDelete.Enabled then
    actDetailDelete.Execute;
end;

procedure Tgdc_frmSetting.actCopyToSettingUpdate(Sender: TObject);
begin
  actCopyToSetting.Enabled := gdcSettingPos.Active
    and (gdcSettingPos.RecordCount > 0);
end;

procedure Tgdc_frmSetting.actMoveToSettingUpdate(Sender: TObject);
begin
  actMoveToSetting.Enabled := gdcSettingPos.Active
    and (gdcSettingPos.RecordCount > 0);
end;

function Tgdc_frmSetting.CopySettingPos: Boolean;
var
  ObjID: Integer;
  Obj: TgdcBase;
  Cl: TPersistentClass;
  AddDlg: TdlgToSetting;
begin
  Result := False;

  ObjID := gdcBaseManager.GetIDByRUID(
    gdcSettingPos.FieldByName('xid').AsInteger, gdcSettingPos.FieldByName('dbid').AsInteger);
  if ObjID > -1 then
  begin
    Cl := GetClass(gdcSettingPos.FieldByName('objectclass').AsString);
    if Assigned(Cl) and Cl.InheritsFrom(TgdcBase) then
    begin
      Obj := CgdcBase(Cl).CreateWithID(nil, nil, nil,
        ObjID,
        gdcSettingPos.FieldByName('subtype').AsString);
      try
        Obj.Open;
        if not Obj.IsEmpty then
        begin
          AddDlg := TdlgToSetting.Create(Self);
          try
            AddDlg.Setup(False, '', '', Obj, GetDetailBookmarkList);
            Result := (AddDlg.ShowModal = mrOk);
          finally
            FreeAndNil(AddDlg);
          end;
        end;
