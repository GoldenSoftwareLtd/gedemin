...
type
  ...
  protected
    ...
    procedure ExecuteDDLStatement(const ASQL: String); overload;
    procedure ExecuteDDLStatement(const ASQL: TStrings); overload;
...
begin
  ...
  ExecuteDDLStatement(Format('DROP DOMAIN %s', [FieldByName('fieldname').AsString]));
...
begin
  ...
  if AnsiPos(UserPrefix, AnsiUpperCase(Trim(FieldByName('fieldname').AsString))) = 1 then
    ExecuteDDLStatement(Format('CREATE DOMAIN %s AS %s', [FieldByName('fieldname').AsString, GetDomainText]));
...
procedure TgdcMetaBase.ExecuteDDLStatement(const ASQL: String);
var
  ibsql: TIBSQL;
  DidActivate: Boolean;
begin
...
procedure TgdcMetaBase.ExecuteDDLStatement(const ASQL: TStrings);
var
  I: Integer;
  ibsql: TIBSQL;
  DidActivate: Boolean;
