...
type
  ...
  // constructor Create(AnOwner: TComponent); override;
    ...
    procedure DoBeforeActivation(const DontHideForms: Boolean);
    procedure DoAfterActivation(const DontHideForms: Boolean);
  ...
  {
    Больше не используется, работаем с хранилищем через TgdcSettingPos
  }  
...
// список объектов с информацией о файле настройки
  ...
  public
    ...
    procedure LoadPackageInfo;    // можно добавить пар-тр - forEndingOnly
  ...
  procedure _ActivateExternalSetting(const ASettingFileNameStr: String; const DontHideForms: Boolean; AnAnswer: Word = mrNone);
...
begin
  // Если формат настройки не задан, попытаемся определить по блобу данных
  ...
  begin
    ...
    // Если и сейчас не смогли определить формат, значит настройка пуста, сохраним в XML
    if StreamFormat = sttUnknown then
      StreamFormat := sttXML;
  end;
  ...
  begin
    ...
    begin
      ...
      try
        ...
        // Шапка и позиции настройки
        StreamSaver.AddSetting(Self);
        // Сохранить данные в файл
        StreamSaver.SaveToFile(FN);
      finally
        StreamSaver.Free;
      end;
    end
...
begin
  ...
  begin
    ...
    try
      ...
      begin
        Self.DoBeforeActivation(Self.Silent);
        try
          _ActivateExternalSetting(FN, Self.Silent);
        finally
          Self.DoAfterActivation(Self.Silent);
        end;
      end
      else
      begin
...
begin
  ...
  begin
    // Формирование настройки для нового потока перенесено в SaveToFile
    Self.Edit;
    Self.FieldByName('data').Clear;
    Self.FieldByName('storagedata').Clear;
    Self.Post;
  end
  else
  begin
...
procedure TgdcSetting.DoBeforeActivation(const DontHideForms: Boolean);
begin
  //... и сохранить хранилища
  if Assigned(GlobalStorage) and GlobalStorage.IsModified then
    GlobalStorage.SaveToDatabase;
  if Assigned(UserStorage) and UserStorage.IsModified then
    UserStorage.SaveToDatabase;
  if Assigned(CompanyStorage) and CompanyStorage.IsModified then
    CompanyStorage.SaveToDatabase;

  DesktopManager.WriteDesktopData(cst_DesktopLast, True);
  // Если не стоит флажок "не закрывать формы", то закроем все формы
  if not DontHideForms then
    FreeAllForms(False);
end;

procedure TgdcSetting.DoAfterActivation(const DontHideForms: Boolean);
begin
  if not DontHideForms then
  begin
      ...
      begin
        try
          ...
          begin
            ...
            begin
              // Если пользователь не указал удалять все, то спросим об удалении текущего объекта
              if DeleteAnswer <> mrYesToAll then
                DeleteAnswer := MessageDlg(' Ошибка при считывании объекта ' +
                  FieldByName('category').AsString + ' ' +
                  FieldByName('objectname').AsString + #13#10 +
                  ' (с XID = ' + FieldByName('xid').AsString +
                  '   DBID = ' + FieldByName('dbid').AsString + ')' + #13#10 +
                  'Удалить позицию настройки?', mtConfirmation,
                  [mbYes, mbYesToAll, mbNo, mbNoToAll], 0);
              // Если пользователь подтвердил удаление объекта
              if DeleteAnswer in [mrYes, mrYesToAll] then
              begin
                Delete;
                WasDelete := True;
              end
              else if DeleteAnswer = mrNoToAll then      // Если пользователь указал ничего не удалять, то выходим из цикла
                Break;
            end;
          end;
        end;
...
function TGSFList.LoadInfoFor(aID: Integer; const aRUID, aPath: String; var Err: String;                       
  var MaxPackVerInfo: TSettingVersion; out isCorrect: Boolean): TSettingVersion;
var 
  k, ind: Integer;
...
begin
  ...
  begin 
    ...
    if ((Objects[ind] as TGSFHeader).VerInfo = svIndefinite) or
      ((Objects[ind] as TGSFHeader).MaxVerInfo < (Objects[ind] as TGSFHeader).VerInfo) then      // новизна не определена
    begin
...
procedure TGSFList.LoadPackageInfo;
var
  i: Integer;
  CP: Boolean;
...
begin
  ...
  begin
    ...
    if (Ending = 1) and                      
       ((VerInfo = svIndefinite)) then
    begin
      ErrMsg := '';
      MaxVer := svOlder;
      LoadInfoFor(i, Strings[i], '', ErrMsg, MaxVer, CP);  // Strings[i] хранит RUID
      MaxVerInfo := MaxVer;
      ErrMessage := ErrMsg;
    end;
  end;
...
begin
  ...
  gdcSetts.DoBeforeActivation(False);
  try
    SettIDList.Clear;
    ...
    gdcSetts.DoAfterActivation(False);
  end;
end;
...
var
  IDList: TStringList;                 // список настроек текущего пакета
  YesToAll: Integer;
  SettingCounter: Integer;
  CurHeader: TGSFHeader;
            ...
            if ( (Objects[ind] as TGSFHeader).FullCorrect) then
              begin               // промежуточная конечная %)
                ...
                begin             // ищем самую последнюю версию
                  if (Objects[ind] as TGSFHeader).Version > mr then
                  begin
                    i := ind;
                    mr := (Objects[ind] as TGSFHeader).Version;
                  end;
                end else
...
// +++ позволяем пользователю выбрать пакет +++
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            begin
              // добавляем настройку в список_для_установки
              IDList.Add(IntToStr(ind));
            end;
    ...
    else       // настройка _не_ найдена в списке
    begin
      RUID := StrToRUID(aRUID);
      RealSettID := gdcBaseManager.GetIDByRUID(RUID.XID, RUID.DBID);
      if RealSettID > -1 then
...
begin                                      
  ...
  try
    ...
    begin
      for SettingCounter := 0 to IDList.Count - 1 do
      begin
        CurHeader := Objects[StrToInt(IDList[SettingCounter])] as TGSFHeader;
        if CurHeader.StreamType <> sttBinaryOld then
        begin
          _ActivateExternalSetting(CurHeader.FilePath + CurHeader.FileName, False, YesToAll);
        end
        else
        begin
          gdcSetts.Open;
          gdcSetts.LoadFromFile(CurHeader.FilePath + CurHeader.FileName);
          gdcSetts.Close;
          if LastLoadedSettingKey <> -1 then
            _ActivateSetting(IntToStr(LastLoadedSettingKey), False, True, YesToAll, YesToAll);
        end;
      end;
      Result := True;
    end        
...
begin
  ...
  try
    ...
    if StreamType <> sttBinaryOld then
    begin
      SetLength(RUIDString, BlobStream.Size);
      BlobStream.ReadBuffer(RUIDString[1], BlobStream.Size);
      RuidList.CommaText := RUIDString;
    end
    else
    begin
...
var
  ...
  procedure DoSettingLog(const ASettingName: String);
  var
    J: DWORD;
    TempPath: array[0..MAX_COMPUTERNAME_LENGTH] of Char;
  begin
    // Логгирование загрузки настроек
    ...
    begin
      ...
      begin
        TgdcJournal.AddEvent(
          'Загружена и активирована настройка "' + ASettingName + '". С компьютера ' + StrPas(TempPath) + '.',
          'TgdcSetting', -1, nil, True);
      end;
    end;
  end;

begin
  // Убедимся что указана хотя бы одна настройка для активации
  if ASettingFileNameStr = '' then
    Exit;

  // Сюда будем записывать текст ошибки, если таковая возникнет при загрузке настройки
  ...
  try
    SettingList.Text := ASettingFileNameStr;
    ...
    try
      ...
      try
        ...
        // Идем по переданному списку настроек, и активируем их
        for StNumber := 0 to SettingList.Count - 1 do
        begin
          // Загрузим данные настройки из файла
          StreamSaver.LoadFromFile(SettingList[StNumber]);
          // Активируем настройку - загрузим данные из памяти в БД
          StreamSaver.ActivateSetting(WasMetaDataInSetting, AnAnswer);
          // Подготовим загрузчик к след. настройке
          StreamSaver.Clear;
          ...
          // Логгирование загрузки настроек
          DoSettingLog(SettingList.Strings[StNumber]);
        end;
      finally
        FreeAndNil(StreamSaver);
      end;
    finally
      try
        if not DontHideForms then
