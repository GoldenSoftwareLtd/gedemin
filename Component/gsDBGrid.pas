...
//    procedure SaveToStream(Stream: TStream);
    ...
    procedure LoadFromStreamXML(Stream: TStream);
    procedure SaveToStreamXML(Stream: TStream);
...
procedure TgsCustomDBGrid.LoadFromStreamXML(Stream: TStream);
type
  TXMLElementType = (etUnknown, etGrid, etFont, etOptions, etExpandSet, etExpand, etConditionSet, etCondition, etColumnSet, etColumn, etTitle);
var
  XMLReader: Tgd_XMLReader;
  XMLElement: TgsXMLElement;
  FontTypeStr: String;
  ParentElement: TXMLElementType;
  CurrentExpand: TColumnExpand;
  CurrentCondition: TCondition;
  CurrentColumn: TgsColumn;
  CurrentTitle: TgsColumnTitle;

  function ReadColor(ColorStr: String; DefColor: TColor): TColor;
  begin
    try
  ...
  function GetElementType(const ATagStr: String): TXMLElementType;
  begin
    if AnsiCompareText(ATagStr, 'GRID') = 0 then
      Result := etGrid
    else if AnsiCompareText(ATagStr, 'FONT') = 0 then
      Result := etFont
    else if AnsiCompareText(ATagStr, 'OPTIONS') = 0 then
      Result := etOptions
    else if AnsiCompareText(ATagStr, 'EXPAND_SET') = 0 then
      Result := etExpandSet
    else if AnsiCompareText(ATagStr, 'EXPAND') = 0 then
      Result := etExpand
    else if AnsiCompareText(ATagStr, 'CONDITION_SET') = 0 then
      Result := etConditionSet
    else if AnsiCompareText(ATagStr, 'CONDITION') = 0 then
      Result := etCondition
    else if AnsiCompareText(ATagStr, 'COLUMN_SET') = 0 then
      Result := etColumnSet
    else if AnsiCompareText(ATagStr, 'COLUMN') = 0 then
      Result := etColumn
    else if AnsiCompareText(ATagStr, 'TITLE') = 0 then
      Result := etTitle
    else
      Result := etUnknown;
  end;

  procedure ReadFont(Font: TFont);
  var
    CurStyle: TFontStyles;
  begin
    Font.Name := XMLReader.GetParamValueByName(XMLElement, 'name');
    Font.Color := ReadColor(XMLReader.GetParamValueByName(XMLElement, 'color'), Font.Color);
    Font.Height := XMLReader.GetIntegerParamValueByName(XMLElement, 'height');
    Font.Pitch := TFontPitch(XMLReader.GetIntegerParamValueByName(XMLElement, 'pitch'));                      // TODO
    Font.Size := XMLReader.GetIntegerParamValueByName(XMLElement, 'size');
    Font.CharSet := XMLReader.GetIntegerParamValueByName(XMLElement, 'charset');

    if Boolean(XMLReader.GetIntegerParamValueByName(XMLElement, 'bold')) then
      Include(CurStyle, fsBold);
    if Boolean(XMLReader.GetIntegerParamValueByName(XMLElement, 'italic')) then
      Include(CurStyle, fsItalic);
    if Boolean(XMLReader.GetIntegerParamValueByName(XMLElement, 'underline')) then
      Include(CurStyle, fsUnderline);
    if Boolean(XMLReader.GetIntegerParamValueByName(XMLElement, 'strikeout')) then
      Include(CurStyle, fsStrikeOut);
    Font.Style := CurStyle;
  end;

  procedure ReadOptions;
  var
    ReadOpt: TDBGridOptions;
  begin
    if Boolean(XMLReader.GetIntegerParamValueByName(XMLElement, 'Editing')) then
      Include(ReadOpt, dgEditing);
    if Boolean(XMLReader.GetIntegerParamValueByName(XMLElement, 'AlwaysShowEditor')) then
      Include(ReadOpt, dgAlwaysShowEditor);
    if Boolean(XMLReader.GetIntegerParamValueByName(XMLElement, 'Titles')) then
      Include(ReadOpt, dgTitles);
    if Boolean(XMLReader.GetIntegerParamValueByName(XMLElement, 'Indicator')) then
      Include(ReadOpt, dgIndicator);
    if Boolean(XMLReader.GetIntegerParamValueByName(XMLElement, 'ColLines')) then
      Include(ReadOpt, dgColLines);
    if Boolean(XMLReader.GetIntegerParamValueByName(XMLElement, 'RowLines')) then
      Include(ReadOpt, dgRowLines);
    if Boolean(XMLReader.GetIntegerParamValueByName(XMLElement, 'Tabs')) then
      Include(ReadOpt, dgTabs);
    if Boolean(XMLReader.GetIntegerParamValueByName(XMLElement, 'RowSelect')) then
      Include(ReadOpt, dgRowSelect);
    if Boolean(XMLReader.GetIntegerParamValueByName(XMLElement, 'AlwaysShowSelection')) then
      Include(ReadOpt, dgAlwaysShowSelection);
    if Boolean(XMLReader.GetIntegerParamValueByName(XMLElement, 'ConfirmDelete')) then
      Include(ReadOpt, dgConfirmDelete);
    if Boolean(XMLReader.GetIntegerParamValueByName(XMLElement, 'CancelOnExit')) then
      Include(ReadOpt, dgCancelOnExit);
    if Boolean(XMLReader.GetIntegerParamValueByName(XMLElement, 'MultiSelect')) then
      Include(ReadOpt, dgMultiSelect);

    Self.Options := ReadOpt;
  end;

  procedure ReadExpand;
  var
    Opt: TColumnExpandOptions;
  begin
    if Assigned(CurrentExpand) then
    begin
      CurrentExpand.DisplayField := XMLReader.GetParamValueByName(XMLElement, 'displayfield');
      CurrentExpand.FieldName := XMLReader.GetParamValueByName(XMLElement, 'fieldname');
      CurrentExpand.LineCount := XMLReader.GetIntegerParamValueByName(XMLElement, 'linecount');
      CurrentExpand.LineCount := XMLReader.GetIntegerParamValueByName(XMLElement, 'linecount');
      if Boolean(XMLReader.GetIntegerParamValueByName(XMLElement, 'addfield')) then
        Include(Opt, ceoAddField);
      if Boolean(XMLReader.GetIntegerParamValueByName(XMLElement, 'addfieldmultiline')) then
        Include(Opt, ceoAddFieldMultiline);
      if Boolean(XMLReader.GetIntegerParamValueByName(XMLElement, 'multiline')) then
        Include(Opt, ceoMultiline);
      CurrentExpand.Options := Opt;
    end;
  end;

  procedure ReadCondition;
  begin
    if Assigned(CurrentCondition) then
    begin
  ...
  begin
    if Assigned(CurrentColumn) then
    begin
...
begin
  ...
  try
    ...
    ParentElement := etUnknown;

    while XMLReader.GetNextElement(XMLElement) do
    begin
      if XMLElement.Kind = epOpening then
      begin
        case GetElementType(XMLElement.Tag) of
          etGrid:
          begin
            TableColor := ReadColor(XMLReader.GetParamValueByName(XMLElement, 'table_color'), TableColor);
            SelectedColor := ReadColor(XMLReader.GetParamValueByName(XMLElement, 'selected_color'), SelectedColor);
            TitleColor := ReadColor(XMLReader.GetParamValueByName(XMLElement, 'title_color'), TitleColor);
            Striped := Boolean(XMLReader.GetIntegerParamValueByName(XMLElement, 'stripped'));
            StripeOdd := ReadColor(XMLReader.GetParamValueByName(XMLElement, 'stripe_odd_color'), StripeOdd);
            StripeEven := ReadColor(XMLReader.GetParamValueByName(XMLElement, 'stripe_even_color'), StripeEven);
            FShowTotals := Boolean(XMLReader.GetIntegerParamValueByName(XMLElement, 'show_totals'));
            FShowFooter := Boolean(XMLReader.GetIntegerParamValueByName(XMLElement, 'show_footer'));

            ParentElement := etGrid;
          end;
          ...
          begin
            case ParentElement of
              etGrid:
              begin
                FontTypeStr := XMLReader.GetParamValueByName(XMLElement, 'type');
                if AnsiCompareText(FontTypeStr, 'table_font') = 0 then
                  ReadFont(TableFont)
                else if AnsiCompareText(FontTypeStr, 'selected_font') = 0 then
                  ReadFont(SelectedFont)
                else if AnsiCompareText(FontTypeStr, 'title_font') = 0 then
                  ReadFont(TitleFont);
              end;
              ...
              begin
                if Assigned(CurrentCondition) then
                  ReadFont(CurrentCondition.Font);
              end;

              etColumn:
              begin
                if Assigned(CurrentColumn) then
                  ReadFont(CurrentColumn.Font);
              end;

              etTitle:
              begin
                if Assigned(CurrentTitle) then
                  ReadFont(CurrentTitle.Font);
              end;
            end;
          end;
          ...
          etExpandSet:
          begin
            ExpandsActive := Boolean(XMLReader.GetIntegerParamValueByName(XMLElement, 'active'));
            ExpandsSeparate := Boolean(XMLReader.GetIntegerParamValueByName(XMLElement, 'separate'));
          end;

          etExpand:
          begin
            CurrentExpand := Expands.Add;
            ReadExpand;
          end;

          etConditionSet:
          begin
            ConditionsActive := Boolean(XMLReader.GetIntegerParamValueByName(XMLElement, 'active'));
          end;

          etCondition:
          begin
            CurrentCondition := Conditions.Add;
            ReadCondition;
            ParentElement := etCondition;
          end;

          etColumnSet:
          begin
            ScaleColumns := Boolean(XMLReader.GetIntegerParamValueByName(XMLElement, 'scale'));
            TitlesExpanding := Boolean(XMLReader.GetIntegerParamValueByName(XMLElement, 'titles_expanding'));
          end;

          etColumn:
          begin
            CurrentColumn := TgsColumn(Columns.Add);
            ReadColumn;
            CurrentTitle := TgsColumnTitle(CurrentColumn.Title);
            ParentElement := etColumn;
          end;

          etTitle:
          begin
            ReadTitle;
            ParentElement := etTitle;
          end;
        end;
      end;
    end;
...
procedure TgsCustomDBGrid.SaveToStreamXML(Stream: TStream);
var
  XMLWriter: Tgd_XMLWriter;
  I: Integer;

  procedure WriteFont(AFont: TFont; const FontTypeStr: String = '');
  begin
    if FontTypeStr <> '' then
      XMLWriter.AddAttribute('type', FontTypeStr);
    XMLWriter.AddAttribute('name', AFont.Name);
    XMLWriter.AddAttribute('color', ColorToString(AFont.Color));
    XMLWriter.AddAttribute('height', IntToStr(AFont.Height));
    XMLWriter.AddAttribute('pitch', IntToStr(Integer(AFont.Pitch)));
    XMLWriter.AddAttribute('size', IntToStr(AFont.Size));
    XMLWriter.AddAttribute('charset', IntToStr(AFont.CharSet));
    XMLWriter.AddAttribute('bold', IntToStr(Integer(fsBold in AFont.Style)));
    XMLWriter.AddAttribute('italic', IntToStr(Integer(fsItalic in AFont.Style)));
    XMLWriter.AddAttribute('underline', IntToStr(Integer(fsUnderline in AFont.Style)));
    XMLWriter.AddAttribute('strikeout', IntToStr(Integer(fsStrikeOut in AFont.Style)));
    XMLWriter.AddShortElement('font');
  end;

  procedure WriteExpand(Expand: TColumnExpand);
  begin
    XMLWriter.AddAttribute('displayfield', Expand.DisplayField);
    XMLWriter.AddAttribute('fieldname', Expand.FieldName);
    XMLWriter.AddAttribute('linecount', IntToStr(Expand.LineCount));
    XMLWriter.AddAttribute('addfield', IntToStr(Integer(ceoAddField in Expand.Options)));
    XMLWriter.AddAttribute('addfieldmultiline', IntToStr(Integer(ceoAddFieldMultiline in Expand.Options)));
    XMLWriter.AddAttribute('multiline', IntToStr(Integer(ceoMultiline in Expand.Options)));
    XMLWriter.AddShortElement('expand');
  end;

  procedure WriteCondition(Condition: TCondition);
  begin
    XMLWriter.AddAttribute('conditionname', Condition.ConditionName);
    XMLWriter.AddAttribute('displayfields', Condition.DisplayFields);
    XMLWriter.AddAttribute('fieldname', Condition.FieldName);
    XMLWriter.AddAttribute('expression1', Condition.Expression1);
    XMLWriter.AddAttribute('expression2', Condition.Expression2);
    XMLWriter.AddAttribute('conditionkind', IntToStr(Integer(Condition.ConditionKind)));          // TODO
    XMLWriter.AddAttribute('evaluateformula', IntToStr(Integer(Condition.EvaluateFormula)));
    XMLWriter.AddAttribute('usercondition', IntToStr(Integer(Condition.UserCondition)));
    XMLWriter.AddAttribute('color', ColorToString(Condition.Color));
    XMLWriter.AddAttribute('usecolor', IntToStr(Integer(doColor in Condition.DisplayOptions)));
    XMLWriter.AddAttribute('usefont', IntToStr(Integer(doFont in Condition.DisplayOptions)));
    XMLWriter.AddOpenTag('condition');

    WriteFont(Condition.Font);

    XMLWriter.AddCloseTag('condition');
  end;

  procedure WriteColumn(Column: TgsColumn);
  begin
    XMLWriter.AddAttribute('fieldname', Column.Fieldname);
    XMLWriter.AddAttribute('alignment', IntToStr(Integer(Column.Alignment)));       // TODO
    XMLWriter.AddAttribute('buttonstyle', IntToStr(Integer(Column.ButtonStyle)));   // TODO
    XMLWriter.AddAttribute('color', ColorToString(Column.Color));
    XMLWriter.AddAttribute('displayformat', Column.DisplayFormat);
    XMLWriter.AddAttribute('dropdownrows', IntToStr(Column.DropDownRows));
    XMLWriter.AddAttribute('expanded', IntToStr(Integer(Column.Expanded)));
    XMLWriter.AddAttribute('frozen', IntToStr(Integer(Column.Frozen)));
    XMLWriter.AddAttribute('picklist', Column.Picklist.Text);
    XMLWriter.AddAttribute('readonly', IntToStr(Integer(Column.Readonly)));
    XMLWriter.AddAttribute('total', IntToStr(Integer(Column.TotalType)));          // TODO
    XMLWriter.AddAttribute('visible', IntToStr(Integer(Column.Visible)));
    XMLWriter.AddAttribute('width', IntToStr(Column.Width));
    XMLWriter.AddOpenTag('column');

    WriteFont(Column.Font);

    XMLWriter.AddAttribute('alignment', IntToStr(Integer(Column.Title.Alignment)));       // TODO
    XMLWriter.AddAttribute('caption', Column.Title.Caption);
    XMLWriter.AddAttribute('color', ColorToString(Column.Title.Color));
    XMLWriter.AddOpenTag('title');
    WriteFont(Column.Title.Font);
    XMLWriter.AddCloseTag('title');

    XMLWriter.AddCloseTag('column');
  end;
...
begin
  ...
  try
    ...
    XMLWriter.AddAttribute('table_color', ColorToString(TableColor));
    XMLWriter.AddAttribute('selected_color', ColorToString(SelectedColor));
    XMLWriter.AddAttribute('title_color', ColorToString(TitleColor));
    XMLWriter.AddAttribute('stripped', IntToStr(Integer(Striped)));
    XMLWriter.AddAttribute('stripe_odd_color', ColorToString(StripeOdd));
    XMLWriter.AddAttribute('stripe_even_color', ColorToString(StripeEven));
    XMLWriter.AddAttribute('show_totals', IntToStr(Integer(FShowTotals)));
    XMLWriter.AddAttribute('show_footer', IntToStr(Integer(FShowFooter)));
    XMLWriter.AddOpenTag('grid');

    WriteFont(TableFont, 'table_font');
    WriteFont(SelectedFont, 'selected_font');
    WriteFont(TitleFont, 'title_font');

    XMLWriter.AddAttribute('Editing', IntToStr(Integer(dgEditing in Options)));
    XMLWriter.AddAttribute('AlwaysShowEditor', IntToStr(Integer(dgAlwaysShowEditor in Options)));
    XMLWriter.AddAttribute('Titles', IntToStr(Integer(dgTitles in Options)));
    XMLWriter.AddAttribute('Indicator', IntToStr(Integer(dgIndicator in Options)));
    XMLWriter.AddAttribute('ColumnResize', IntToStr(Integer(dgColumnResize in Options)));
    XMLWriter.AddAttribute('ColLines', IntToStr(Integer(dgColLines in Options)));
    XMLWriter.AddAttribute('RowLines', IntToStr(Integer(dgRowLines in Options)));
    XMLWriter.AddAttribute('Tabs', IntToStr(Integer(dgTabs in Options)));
    XMLWriter.AddAttribute('RowSelect', IntToStr(Integer(dgRowSelect in Options)));
    XMLWriter.AddAttribute('AlwaysShowSelection', IntToStr(Integer(dgAlwaysShowSelection in Options)));
    XMLWriter.AddAttribute('ConfirmDelete', IntToStr(Integer(dgConfirmDelete in Options)));
    XMLWriter.AddAttribute('CancelOnExit', IntToStr(Integer(dgCancelOnExit in Options)));
    XMLWriter.AddAttribute('MultiSelect', IntToStr(Integer(dgMultiSelect in Options)));
    XMLWriter.AddShortElement('options');

    // Расширенное отображение
    if FExpands.Count > 0 then
    begin
      XMLWriter.AddAttribute('active', IntToStr(Integer(ExpandsActive)));
      XMLWriter.AddAttribute('separate', IntToStr(Integer(ExpandsSeparate)));
      XMLWriter.AddOpenTag('expand_set', True);
      for I := 0 to FExpands.Count - 1 do
        WriteExpand(FExpands.Items[I]);
      XMLWriter.AddCloseTag('expand_set');
    end;

    // Условия
    if FConditions.Count > 0 then
    begin
      XMLWriter.AddAttribute('active', IntToStr(Integer(ConditionsActive)));
      XMLWriter.AddOpenTag('condition_set', True);
      for I := 0 to FConditions.Count - 1 do
        WriteCondition(FConditions.Items[I]);
      XMLWriter.AddCloseTag('condition_set');
    end;

    // Колонки
    if Columns.Count > 0 then
    begin
      XMLWriter.AddAttribute('scale', IntToStr(Integer(ScaleColumns)));
      XMLWriter.AddAttribute('titles_expanding', IntToStr(Integer(TitlesExpanding)));
      XMLWriter.AddOpenTag('column_set', True);
      for I := 0 to Columns.Count - 1 do
        WriteColumn(TgsColumn(Columns.Items[I]));
      XMLWriter.AddCloseTag('column_set');
    end;  

    XMLWriter.AddCloseTag('grid');
  finally
