...
type
  ...
  public
    ...
    function SaveComponentXML(C: TComponent; M: TSaveToStreamMethod; const Context: String = ''): String;
    procedure LoadComponentXML(C: TComponent; M: TLoadFromStreamMethod; const Context: String = '';
      const ALoadAdmin: Boolean = True);
...
procedure TgsStorage.LoadComponentXML(C: TComponent;
  M: TLoadFromStreamMethod; const Context: String;
  const ALoadAdmin: Boolean);
...
function TgsStorage.SaveComponentXML(C: TComponent; M: TSaveToStreamMethod;
...
begin
  ...
  begin
    ...
    try
      ...
      if Assigned(F) then
      try
        if not F.ValueExists('dataXML') then
          F.WriteStream('dataXML', nil);
        V := F.ValueByName('dataXML');
        if V is TgsStreamValue then
          V.AsString := SNew.DataString;
      finally
