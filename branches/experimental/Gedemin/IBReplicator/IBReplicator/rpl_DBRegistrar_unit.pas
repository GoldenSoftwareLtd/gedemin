unit rpl_DBRegistrar_unit;

interface
uses rpl_BaseTypes_unit, Registry, rpl_ResourceString_unit, SysUtils,
  Classes, rpl_const;
...
begin
  ...
  b:= False;
  for i:= 0 to FServerList.Count - 1 do begin
    b:= AnsiUpperCase(Trim(FServerList[i])) = 'LOCALHOST';
    if b then Break;
  end;
  if not b then
    FServerList.Add('localhost');
  Result := FServerList;
end;
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        if not R.ValueExists(FAlias) then
          raise Exception.Create(CantLoadRegistryInfo);
...
begin
  ...
  begin
    ...
    FRole := '';
    FCharSet := '';
    FAdditional := 'NO_GARBAGE_COLLECT';
  end;
  DoChange;
end;
