...
begin
  ...
  begin
    ...
    {Проверяем, является ли первичный ключ простым, является ли он ссылкой.
     Если да, то проверяем, является ли он ссылкой на таблицу}
      ...
      try
        ...
        begin
          ...
          begin
            ...
            begin
              // Ссылка на объект master
              DetailField := Self.DetailLinks[I].FindField(
                Self.DetailLinks[I].GetFieldNameComparedToParam(Self.DetailLinks[I].DetailField));
              // Укажем что объект находится в состоянии копирования
              ...
              try
                ...
                begin
                  ...
                  try
                    ...
                    if Assigned(DetailField) and (DetailField.AsInteger <> Self.ID) then
      ...
      begin
        ...
        begin
          ...
          raise EgdcException.CreateObj(
            ...
            'возможно, в настройку включен документ, но не включены'#13#10 +
            'относящиеся к нему таблицы. Еще одна возможная причина, это'#13#10 +
            'неверно расставленные зависимости между настройками.',
            Self);
        end;
