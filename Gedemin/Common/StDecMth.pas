...
type
  ...
  TStDecimal = class
    ...
    public
      ...
      function _Mod(N: Integer): Integer;
...
function TStDecimal._Mod(N: Integer): Integer;
var
  S: AnsiString;
  P: Integer;
begin
  ...
  if P > 0 then
    SetLength(S, P - 1);
  Result := StrToInt64(S) mod N;
end;

function TStDecimal.DecDigits: Integer;
var
  S: AnsiString;
  B, E: Integer;
begin
  ...
  if S = '' then
    ...
    B := 1;
    while (B < Length(S)) and (S[B] <> DecimalSeparator) do
      Inc(B);
    E := Length(S);
    while (E > B) and (S[E] = '0') do
      Dec(E);
    Result := E - B;
  end;
end;
