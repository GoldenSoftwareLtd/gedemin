...
begin
  ...
  if FInChoose and (FgdcLinkChoose = gdcObject) then
    try
      ...
      raise Exception.Create('ќшибка при выборе записи. Ќеверный id: '+ TID2S(CheckID));
    end;
end;
...
initialization
  RegisterFrmClass(Tgdc_frmMDVTree, 'Master-detail форма с деревом и таблицей').ShowInFormEditor := True;
