...
type
  ...
  TgdcState = (
    ...
    sSubProcess      // в мастере идет некоторый процесс, который вызывает действия в
                     // в нашем детальном объекте. Например, удаляется запись из мастера,
                     // в процессе ее удаления удаляются детальные записи.
                     // в таком случае у детального объекта выставляется флаг.
  ...
  try
    ...
    begin
      ...
      if IBLogin.IsIBUserAdmin and (AMessage > '') then
        ...
        S := S + 'Вероятно, на данную запись ссылаются другие записи.';
