...
//    FSavedAdjustRect: TRect;
    ...
    FTabIndex: Integer;
    //Индекс первой отображаемой страницы
...
function _GetTabsColor(C: TColor): TColor;
var
 RGB: Longint;
begin
  RGB := ColorToRGB(C);
  RGB := (RGB and $FF0000) + ColorDelta shl 16  + (RGB and $FF00) + ColorDelta shl 8 +
    (RGB and $FF) + ColorDelta;
  Result := RGB;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        FTabIndex := FActivePage.TabIndex;
        Exit;
      end;
    end;
    if Page <> nil then
...
//    PostMessage(Handle, WM_PAINT, Canvas.Handle, 0);
    Invalidate;
    InvalidateTabs;
  end;
end;
...
begin
  for I := 0 to FPages.Count - 1 do
    if (TSuperTabSheet(FPages[i]).Visible) and
      IsAccel(Message.CharCode, TSuperTabSheet(FPages[I]).Caption) and
...
begin
  with Message do
  begin
    if (CharCode in [VK_LEFT, VK_RIGHT]){ and
...
begin
  ...
  {!!!!}
    Result := FFirstVisibleTabIndex - 1;
...
function TSuperPageControl.GetTabsColor: TColor;
begin
  Result := _GetTabsColor(Color);
end;
...
begin
  ...
  case TabPosition of
    ...
    begin
      Result.Left := FBorder;
      Result.Right := Width - FBorder;
      Result.Bottom := TabHeight + FBorder;
      Result.Top := FBorder;
    end;
{    tpBottom:
    begin
...
procedure TSuperPageControl.MoveTab(CurIndex, NewIndex: Integer);
begin
...
function TSuperTabSheet.GetTabIndex: Integer;
var
  I: Integer;
begin
  Result := 0;
  if not FTabShowing then Dec(Result) else
    for I := 0 to PageIndex - 1 do
      if TSuperTabSheet(FPageControl.FPages[I]).FTabShowing then
        Inc(Result);
end;
...
begin
  inherited;
  C := _GetTabsColor(TSuperPageControl(Owner).Color);
  R := GetClientRect;
  Canvas.Brush.Color := C;
  Canvas.FillRect(R);
  if FMouseOn then
