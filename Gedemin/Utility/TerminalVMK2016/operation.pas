...
begin
  ...
  try
   ...
   // Application.MessageBox(Pchar(BarCode), '', 0);
    ...
    begin
      GetInfoGoods(BarCode, Code, NameGoods, Weight, Date, Number, Npart, NTara, InvWeight, Vname);
     ...
     // eGoods.Text := IntToStr(FPosition.Count - DocumentLine);
      mTP.Lines.Add(NameGoods + '=' + FloatToStrF(WeightInKg/InvWeight, ffFixed, 6, 3) + Vname);
    end;
...
procedure TOperationTP.SetBarCodeSypka(const AKey: String);
var
  Weight, ProductCode, Number: Integer;
  WeightInKg, InvWeight: Double;
  Date: TDateTime;
