...
const
  NEWDOCUMENT_NUMBER = 'á/í';            // íîìåð íîâûõ äîêóìåíòîâ äëÿ áóõ ñàëüäî
  NEWINVDOCUMENT_NUMBER = '1';           // íîìåð íîâûõ äîìåíòîâ äëÿ ñêëàäñêîãî ñàëüäî
  PROIZVOLNYE_TRANSACTION_KEY = 807001;  // AC_TRANSACTION.id WHERE AC_TRANSACTION.name = Ïðîèçâîëüíûå(àÿ) ïðîâîäêè(à)
  PROIZVOLNYE_TRRECORD_KEY = 807100;     // AC_TRRECORD.id WHERE transactionkey = PROIZVOLNYE_TRANSACTION_KEY
  OSTATKY_ACCOUNT_KEY = 300003;          // AC_ACCOUNT.id WHERE fullname = 00 Îñòàòêè
  HOZOPERATION_DOCTYPE_KEY = 806001;     // gd_documenttype.id WHERE name = Õîçÿéñòâåííàÿ îïåðàöèÿ
...
type
  ...
  TOnGetInvCardStatistics = procedure(const InvCardCount: String) of object;
  ...
  private
    ...
    FCardFeaturesStr: String;                                   // cïèñîê ïîëåé-ïðèçíàêîâ ñêëàäñêîé êàðòî÷êè
    FProcessedTbls: TStringList;
    FClosingDate: TDateTime;                                    // äàòà çàêðûòèÿ ïåðèîäà - äî íåå (íå âêëþ÷èòåëüíî) óäàëÿåì äîêóìåíòû
    FCurrentProgressStep: Integer;
    FCurUserContactKey: Integer;
    FDocTypesList: TStringList;                                 // òèïû äîêóìåíòîâ âûáðàííûå ïîëüçîâàòåëåì
    FDoProcDocTypes: Boolean;                                   // true - îáðàáàòûâàòü ÒÎËÜÊÎ äîêóìåíòû ñ âûáðàííûìè òèïàìè, false - îáðàáàòûâàòü âñå ÊÐÎÌÅ äîêóìåíòîâ ñ âûáðàííûìè òèïàìè
    FDoStopProcessing: Boolean;                                 // ôëàã ïðåðûâàíèÿ âûïîëíåíèÿ
    FEntryAnalyticsStr: String;                                 // ñïèñîê âñåõ áóõãàëòåðñêèõ àíàëèòèê
    FInvSaldoDoc: Integer;
    FOurCompaniesListStr: String;                               // ñïèñîê êîìïàíèé èç gd_ourcompany
    //FProizvolnyyDocTypeKey: Integer;                            // ''Ïðîèçâîëüíûé òèï'' èç gd_documenttype
    ...
    FOnGetInvCardStatistics: TOnGetInvCardStatistics;
    ...
    function GetNewID: Integer;                                 // âîçâðàùàåò ñãåíåðèðîâàííûé íîâûé óíèêàëüíûé èäåíòèôèêàòîð
  ...
  public
    ...
    procedure CalculateAcSaldo;                                 // ïîäñ÷åò áóõãàëòåðñêîãî ñàëüäî
    procedure CalculateInvSaldo;                                // ïîäñ÷åò ñêëàäñêèõ îñòàòêîâ
    procedure Connect(ANoGarbageCollect: Boolean; AOffForceWrite: Boolean);
    procedure CreateAcEntries;                                  // ôîðìèðîâàíèå áóõãàëòåðñêîãî ñàëüäî
    procedure CreateDBSStateJournal;                            // ñîçäàíèå òàáëèöû æóðíàëà âûïîëåíèÿ îïåðàöèé, ÷òîáû ïðè ïîâòîðíîé îáðàáîòêå ÁÄ ìîæíî áûëî ïðîäîëæèòü
    procedure CreateHIS_IncludeInHIS;
    procedure CreateInvBalance;
    procedure CreateInvSaldo;                                   // ôîðìèðîâàíèå ñêëàäñêèõ îñòàòêîâ
    procedure CreateMetadata;                                   // ñîçäàíèå íåîáõîäèìûõ òàáëèö äëÿ ïðîãðàììû
    procedure DeleteDBSTables;
    procedure DeleteDocuments_DeleteHIS;
    procedure DeleteOldAcEntryBalance;                          // óäàëåíèå ñòàðîãî áóõ ñàëüäî
    procedure Disconnect;
    procedure DropDBSStateJournal;
    procedure ExecSqlLogEvent(const AnIBQuery: TIBQuery; const AProcName: String); Overload;
    procedure ExecSqlLogEvent(const AnIBSQL: TIBSQL; const AProcName: String); Overload;   // ExecQuery  è çàïèñü â ëîã
    ...
    procedure PrepareDB;                                        // óäàëåíèå PKs, FKs, UNIQs, îòêëþ÷åíèå èíäåêñîâ è òðèããåðîâ
    procedure Reconnect(ANoGarbageCollect: Boolean; AOffForceWrite: Boolean);
    procedure RestoreDB;                                        // âîññòàíîâëåíèå ïåâîíà÷àëüíîãî ñîñòîÿíèÿ (ñîçäàíèå PKs, FKs, UNIQs, âêëþ÷åíèå èíäåêñîâ è òðèããåðîâ)
    procedure SaveMetadata;                                     // ñîõðàíåíèå ïåðâîíà÷àëüíîãî ñîñòîÿíèÿ (PKs, FKs, UNIQs, ñîñòîÿíèÿ èíäåêñîâ è òðèããåðîâ)
    ...
    procedure GetDBPropertiesEvent;                             // ïîëó÷èòü èíôîðìàöèþ î ÁÄ
    procedure GetInvCardFeaturesEvent;                          // çàïîëíèòü ñïèñîê ïðèçíàêîâ INV_CARD äëÿ StringGrid
    procedure GetProcStatisticsEvent;                           // ïîëó÷èòü êîë-âî çàïèñåé äëÿ îáðàáîòêè â GD_DOCUMENT, AC_ENTRY, INV_MOVEMENT
    procedure GetStatisticsEvent;                               // ïîëó÷èòü òåêóùåå êîë-âî çàïèñåé â GD_DOCUMENT, AC_ENTRY, INV_MOVEMENT
    procedure GetInvCardStatisticsEvent;                        // ïîëó÷èòü òåêóùåå êîë-âî çàïèñåé â INV_CARD
    procedure LogEvent(const AMsg: String);                     // çàïèñàòü â ëîã
    ...
    procedure SetDocTypeStringsEvent;                           // çàïîëíèòü ñïèñîê òèïîâ äîêóìåíòîâ äëÿ StringGrid
    procedure UsedDBEvent;                                      // ÁÄ óæå ðàíåå îáðàáàòûâàëàñü ýòîé ïðîãðàììîé, âûâåñòè äèàëîã äëÿ ðåøåíèÿ ïðîäîëæèòü îáðàáîòêó ëèáî íà÷àòü çàíîâî îáðàáàòûâàòü
    ...
    property OnGetInvCardStatistics: TOnGetInvCardStatistics
...
procedure TgsDBSqueeze.CreateUDFs;
  ...
  procedure FuncTest(const AFuncName: String; const ATr: TIBTransaction);     // òåñò íà íàëè÷èå â UDF-ôàéëå ôóíêöèè
  var
    q: TIBSQL;
  begin
    q := TIBSQL.Create(nil);
  ...
  try
    ...
    begin
      q.SQL.Text :=
        ...
        '  STATE         SMALLINT, ' +                                          #13#10 +  // 1-óñïåøíî,0-îøèáêà, NULL-âûïîëíåíèå áûëî ïðåðâàíî ïîëüçîâàòåëåì
      ...
      ExecSqlLogEvent(q, 'CreateDBSStateJournal');
      if q.RecordCount <> 0 then  // ÁÄ óæå îáðàáàòûâàëàñü ïðîãðàììîé
        UsedDBEvent;
      q.Close;
    end;
...
var
  Tr: TIBTransaction;
  q, q2: TIBSQL;
  DocTypeList: TStringList;   // Ñïèñîê òèïîâ äîêóìåíòîâ
  DocTypeBranch: TStringList; // Ñïèñîê âåòâè òèïîâ äîêóìåíòîâ
  I: Integer;
...
begin
  ...
  try
    ...
    begin
      DocTypeList.Append(q.FieldByName('DocType').AsString);
      DocTypeBranch.Add('Íåïðèâÿçàííûå ê âåòâè');
    end;
    q.Close;
...
var
  Tr: TIBTransaction;
  q: TIBSQL;
  CardFeaturesList: TStringList;   // Ñïèñîê ïðèçíàêîâ êàðòî÷åê
begin
  ...
  try
   ...
   q.Close;
    ...
    if q.FieldByName('Kolvo').AsInteger <> 0 then
      WarningEvent('Âíèìàíèå!' + #13#10 + 'Îáíàðóæåíû ïðîâîäêè ñ äàòàìè ac_entry.entrydate < 01.01.1900');
...
//---------------------------------------------------------------------------
procedure TgsDBSqueeze.GetInvCardStatisticsEvent;
var
  q: TIBSQL;
  Tr: TIBTransaction;
begin
  LogEvent('Getting INV_CARD statistics...');
  ProgressMsgEvent('Ïîëó÷åíèå ñòàòèñòèêè...', 0);
  Assert(Connected);
  ...
  try
    ...
    ExecSqlLogEvent(q, 'GetInvCardStatisticsEvent');

    if not FDoStopProcessing then
    begin
      FOnGetInvCardStatistics(q.FieldByName('Kolvo').AsString);

      LogEvent('Getting INV_CARD statistics... OK');
      Tr.Commit;
    end;
    ProgressMsgEvent(' ', 0);
  finally
...
begin
  ...
  try
    ...
    if Result = 1 then
      LogEvent(Format('HIS[%d] ñîçäàí óñïåøíî.', [AnIndex]))
    else begin
      LogEvent(Format('Ïîïûòêà ñîçäàíèÿ HIS[%d] çàâåðøèëàñü íåóäà÷åé!', [AnIndex]));
...
begin
  ...
  try
    ...
    if Result = 1 then
      LogEvent(Format('HIS[%d] ðàçðóøåí óñïåøíî.', [AnIndex]))
    else begin
      LogEvent(Format('Ïîïûòêà ðàçðóøåíèÿ HIS[%d] çàâåðøèëàñü íåóäà÷åé!', [AnIndex]));
    end;
  finally
    q.Free;
    Tr.Free;
...
begin
  ...
  try
    ...
    {q.SQL.Text :=
      ...
      ' WHERE TRIM(gd.name) = ''Ïðîèçâîëüíûé òèï'' ';
    ExecSqlLogEvent(q, 'CreateInvSaldo');

    if q.EOF then
      raise EgsDBSqueeze.Create('Îòñóòñòâóåò çàïèñü GD_DOCUMENTTYPE.NAME = ''Ïðîèçâîëüíûé òèï''');
...
begin
  ...
  try
    ...
    q.SQL.Text :=
      ...
      '  LIST(i.rdb$field_position || ''='' || TRIM(i.rdb$field_name)) ' +                   #13#10 +
      'FROM ' +                                                                              #13#10 +
      '  rdb$relation_constraints c ' +                                                      #13#10 +
      '  JOIN rdb$index_segments i ON c.rdb$index_name = i.rdb$index_name ' +                #13#10 +
      'WHERE ' +                                                                             #13#10 +
    ...
    q.SQL.Text :=
      ...
      '  LIST(iseg.rdb$field_position || ''='' || TRIM(iseg.rdb$field_name))         AS Fields, ' +            #13#10 +
      '  LIST(ref_iseg.rdb$field_position || ''='' || TRIM(ref_iseg.rdb$field_name)) AS Ref_Fields ' +         #13#10 +
      'FROM ' +                                                                                                #13#10 +
      '  rdb$relation_constraints c ' +                                                                        #13#10 +
    ...
    begin
      // äëÿ ññûëî÷íîé öåëîñòíîñòè AcSaldo
      q.SQL.Text :=
        'INSERT INTO DBS_FK_CONSTRAINTS ( ' +                                   #13#10 +
        '  relation_name, ' +                                                   #13#10 +
      ...
      // ÷òîáû ñîõðàíèòü êàðòî÷êè íåîáõîäèìûå InvSaldo
      q.SQL.Text :=
        'INSERT INTO DBS_FK_CONSTRAINTS ( ' +                                   #13#10 +
        '  relation_name, ' +                                                   #13#10 +
        '  ref_relation_name, ' +                                               #13#10 +
        ...
        '  list_fields, list_ref_fields, update_rule, delete_rule) ' +          #13#10 +
        'VALUES( ' +                                                            #13#10 +
        '  ''DBS_TMP_INV_SALDO'', ' +                                           #13#10 +
        '  ''INV_CARD'', ' +                                                    #13#10 +
        '  ''DBS_INV_FK_MOVEMENT_CARDK'', ' +                                   #13#10 +
        '  ''0=CARDKEY'', ''0=ID'', ''RESTRICT'', ''RESTRICT'')';
      ExecSqlLogEvent(q, 'SaveMetadata');
    end;
    Tr.Commit;
...
var
  ...
  Msg: String;
begin
  ProgressMsgEvent('Ïðîâåðêà ÁÄ...', 0);
  LogEvent('Test...');
  Assert(Connected);
  ...
  try
    ...
    if (not q.Eof) then
    begin
      Msg := 'Áûëo îáíàðóæåíî íåñîâïàäåíèå ñàëüäî â inv_balance ñ inv_movement! Ïîäðîáíåå â ëîãå. ';
      LogEvent('Áûëo îáíàðóæåíî íåñîâïàäåíèå ñàëüäî â inv_balance ñ inv_movement! Ïîäðîáíåå â ëîãå. ');
    end;
    ...
    ProgressMsgEvent(' ', 0);
    MsgBoxEvent('Ïðîâåðêà ñîîòâåòñòâèÿ inv_balance è inv_movement çàâåðøåíà.');
  finally
    q.Free;
    Tr.Free;
  end;
...
//---------------------------------------------------------------------------
procedure TgsDBSqueeze.CalculateAcSaldo; // ïîäñ÷åò áóõãàëòåðñêîãî ñàëüäî c ñîõðàíåíèåì â òàáëèöå DBS_TMP_AC_SALDO
var
  Tr: TIBTransaction;
  q2, q3: TIBSQL;
  I: Integer;
  TmpStr: String;
  TmpList: TStringList;
  AvailableAnalyticsList: TStringList;  // cïèñîê àêòèâíûõ àíàëèòèê äëÿ ñ÷åòà
  OurCompany_EntryDocList: TStringList; // ñïèñîê "êîìïàíèÿ=äîêóìåíò äëÿ ïðîâîäîê"
begin
  ...
  try
    ...
    // ñ÷èòàåì è ñîõðàíÿåì ñàëüäî äëÿ êàæäîãî ñ÷åòà
    ...
    begin
      ...
      // ïîëó÷àåì cïèñîê àêòèâíûõ àíàëèòèê, ïî êîòîðûì âåäåòñÿ ó÷åò äëÿ ñ÷åòà
      I := 0;
      ...
      // ïîäñ÷åò ñàëüäî â ðàçðåçå êîìïàíèè, ñ÷åòà, âàëþòû, àíàëèòèê
...
begin
  ...
  try
    ...
    q.SQL.Add(' ' +
      ...
      'WHERE ' +                                                                #13#10 +
      '  im.cardkey > 0 ' +                                                     #13#10 +   // ïåðâûé ñòîëáåö â èíäåêñå, ÷òîáû åãî çàäåéñòâîâàòü
    ...
    q2.SQL.Text :=
      'SELECT FIRST(1) s.companykey FROM DBS_TMP_INV_SALDO s';                  ///TODO: âûáðàòü êîìïàíèþ äëÿ äîêà
...
var
  ...
  begin
    ...
    try
      ...
      try
        ...
        //include HIS_1 - îáÿçàíû îñòàòüñÿ

        //1) ñïèñîê òàáë. ñîñòàâëÿþùèõ åäèíîå öåëîå ñ äîêóìåíòîì

        LineDocTbls.Append('GD_DOCUMENT=PARENT'); // ïîçèöèè
        LineDocTbls.Append('AC_ENTRY=DOCUMENTKEY||MASTERDOCKEY');
        LineDocTbls.Append('AC_RECORD=DOCUMENTKEY||MASTERDOCKEY');

        // òàáë PK=FK             òàáëèöû ñî ñâÿçüþ 1-to-1 ê GD_DOCUMENT

        q.SQL.Text :=
          'SELECT ' +                                                                                                       #13#10 +
          '  TRIM(fc.relation_name)     AS relation_name, ' +                                                               #13#10 +
          '  LIST(TRIM(pc.list_fields)) AS pkfk_field ' +                                                                   #13#10 +
          'FROM dbs_fk_constraints fc ' +                                                                                   #13#10 +
          '  JOIN DBS_SUITABLE_TABLES pc ' +                                                                                #13#10 +
          '    ON pc.relation_name = fc.relation_name ' +                                                                   #13#10 +
          '      AND TRIM(REPLACE(fc.list_fields, ''0='', '''')) = TRIM(pc.list_fields) ' +                                 #13#10 +
          'WHERE ' +                                                                                                        #13#10 +
        ...
        begin
          FkFieldsList.Clear;
          FkFieldsList.Text := StringReplace(q.FieldByName('pkfk_field').AsString, ',', #13#10, [rfReplaceAll, rfIgnoreCase]); // ïîëÿ PK=FK
        ...
        // óïîðÿäî÷èòü ÷òîáû èçáåæàòü íåêîòîðûõ ïåðåîáðàáîòîê: Line2 ññûëàþùèéñÿ íà Line1 äîëæåí ñòîÿòü ÏÅÐÅÄ Line1
        ...
        // 2) cross-òàáë äëÿ ìíîæåñòâ Line
        ...
        begin
          q.SQL.Text :=
            ...
            '  REPLACE(TRIM(fc.list_fields), ''0='', '''') AS cross_main_field ' +     #13#10 +
  ...
  CreateHIS(0);   // çàòÿíóâøèåñÿ äîêóìåíòû - íå íàäî åäèíîå öåëîå ñ íèìè îñòàâëÿòü
  CreateHIS(3);              LogEvent('3)');

  q3.SQL.Text :=
    'SELECT SUM(g_his_include(3, id)) FROM gd_document WHERE parent IS NULL';
  ...
  q3.Close;
      ...
      Tr2.StartTransaction;
        ...
        // 3) include HIS äîêè íà êîòîðûå åñòü ññûëêè (íå îò òàáëèöû Line, íå îò crossLine)
        q2.SQL.Text :=
          'SELECT ' +                                                                #13#10 +
          '  TRIM(fc.relation_name)                            AS relation_name, ' + #13#10 +
          '  LIST(REPLACE(TRIM(fc.list_fields), ''0='', '''')) AS fk_fields ' +      #13#10 +
          'FROM dbs_fk_constraints fc ' +                                            #13#10 +
        ...
        begin
          ...
          begin
            ...
            begin
              q3.SQL.Text :=
                'SELECT ';
              if UpperCase(q2.FieldByName('relation_name').AsString) <> 'DBS_TMP_AC_SALDO' then
                q3.SQL.Add(
                  '  SUM(g_his_include(1, rln.' +  FkFieldsList2[I] + ')) AS Kolvo ')
              else
                q3.SQL.Add(
                  '  SUM(g_his_include(0, rln.' +  FkFieldsList2[I] + ')) AS Kolvo ');

              q3.SQL.Add(
                'FROM '  +                                                      #13#10 +
                   q2.FieldByName('relation_name').AsString + ' rln ' +         #13#10 +
                'WHERE ' +
                '  g_his_has(3, rln.' + FkFieldsList2[I] + ')=1 ');
              ExecSqlLogEvent(q3, 'IncludeDependenciesHIS');
  ...
  DestroyHIS(3);
        ...
        LogEvent('4.)');
        // 4) include linefield Line åñëè íà íåå åñòü ññûëêà (íå îò òàáëèöû Line è íå îò cross-òàáëèö Line)
        for J:=0 to LineDocTbls.Count-1 do
        begin
          // line values
          FkFieldsList.Clear;
          ...
          q2.SQL.Text :=
            ...
            '  TRIM(fc.relation_name)                            AS relation_name, ' +   #13#10 +
            '  LIST(REPLACE(TRIM(fc.list_fields), ''0='', '''')) AS fk_fields, ' +       #13#10 +
            '  REPLACE(TRIM(fc.list_ref_fields), ''0='', '''')   AS list_ref_fields, ' + #13#10 +
            ...
            begin
              ...
              begin
                ...
                begin
                  ...
                  /////////
                  if (UpperCase(q2.FieldByName('relation_name').AsString) = 'DBS_TMP_AC_SALDO') and (LineDocTbls.Names[J] = 'GD_DOCUMENT') then
                  begin
                    q3.SQL.Add('  ' +
                      'SUM(g_his_include(0, rln.' +  FkFieldsList2[I] + ')), ' +    //gd_document.id
                      'SUM(g_his_include(0, line' + IntToStr(I) + '.parent)) ');
                  end
                  /////////
                  else begin
        ...
        // 5) Âñå îñòàâøèåñÿ Line äîëæíû çàòÿíóòü çà ñîáîé äîêè è êðîññ-òàáëèöû, åñëè èìååò ïîëå-ìíîæåñòâî
        ...
        begin
          ...
          //---------îáðàáîòêà FKs Line
          // 5.1) îáðàáîòêà çàïèñåé Line êîòîðûå â HIS
            // Âñå îñòàâøèåñÿ çàïèñè òåêóùåãî Line äîëæíû çàòÿíóòü çà ñîáîé ñâîè FK
          ...
          begin
            ...
            // if PK<147000000 ãëàâíûå ïîëÿ include HIS, ÷òîáû äàëåå çàòÿíóòü FKs òàêèõ çàïèñåé
            // PKs Line
            q4.SQL.Text :=
              'SELECT ' +                                                       #13#10 +
            ...
            begin
              ...
              if AnsiPos(',', q4.FieldByName('pk_fields').AsString) = 0 then
                FkFieldsList2.Text := UpperCase(StringReplace(q4.FieldByName('pk_fields').AsString, '0=', '', [rfReplaceAll, rfIgnoreCase]))
              else
              begin
                //TmpList.Sorted = False;
                FkFieldsList2.Sorted := True;
                FkFieldsList2.Text := UpperCase(StringReplace(q4.FieldByName('pk_fields').AsString, ',', #13#10, [rfReplaceAll, rfIgnoreCase]));

                for I := 0 to FkFieldsList2.Count-1 do
                  TmpList.Append(FkFieldsList2.Values[FkFieldsList2.Names[I]]);
                FkFieldsList2.Clear;
                FkFieldsList2.Sorted := False;
                FkFieldsList2.CommaText := TmpList.CommaText;
              ...
              begin
                ...
                begin
                  RealKolvo := RealKolvo + q3.Fields[I].AsInteger;  // êàæäûé ðàç ïåðåîáðàáîòêà ñ gd_document
          ...
          if LineDocTbls.Names[J] = 'GD_DOCUMENT' then   ///TODO: èçáûòî÷íî - ññûëêè íà ñâîþ òàáë îáðàáàòûâàåì íèæå
          begin
            repeat
              q3.Close;
              q3.SQL.Text :=
          ...
          begin
            ...
            begin
              ...
              else begin// ññûëêà íà ñàìó ñåáÿ - îáðàáîòàåì îòäåëüíî, ÷òîáû èçáåæàòü ïåðåîáðàáîòêè
          ...
          if LineDocTbls.Names[J] = 'INV_CARD' then  // íå îáðàáàòûâàòü çàïèñè äëÿ êîòîðûõ íåò inv_movement - íóæíî óäàëèòü
            ...
            begin                                                LogEvent('5) 1.');
              repeat
                ...
                begin
                  ...
                  FkFieldsList3.Text := StringReplace(LineDocTbls.Values[RefRelation], '||', #13#10, [rfReplaceAll, rfIgnoreCase]); // ãëàâíûå ïîëÿ òàáëèöû ref_relation_name
                  ...
                  if RefRelation = 'GD_DOCUMENT' then
                    ...
                    if RefRelation = 'INV_CARD' then // åñëè åñòü ññûëêà òî íå óäàëÿåì
                      q3.SQL.Add(' + ' +
          ...
          begin
            ...
            begin
              ...
              /////////////
                q3.SQL.Add('  ' +
                  'IIF(g_his_include(0, rln.' +  FkFieldsListLine.Names[I] + ')>0, 0, 0) ');
              /////////////
              end
              else begin
                for K:=0 to FkFieldsList3.Count-1 do
          ...
          // 5.2) if Line ñîäåðæèò ïîëÿ-ìíîæåñòâà, òî îáðàáîòàåì èõ CROSS-òàáëèöû, ÷òîáû ñîõðàíèòü íåîáõîäèìûå çàïèñè

          //---îáðàáîòêà PKs  åñëè òàáë èìååò ïîëÿ-ìíîæåñòâà

          if LineSetTbls.IndexOfName(LineDocTbls.Names[J]) <> -1 then
          begin
            if LineDocTbls.Names[J] <> 'INV_CARD' then /// inv_card.id óæå ñîõðàíèëè - HIS_2
            begin
              // PKs Line     (PK=FK îáðàáîòàíû óæå)
              q4.SQL.Text :=
                'SELECT ' +                                                     #13#10 +
                '  TRIM(c.list_fields) AS pk_fields ' +                         #13#10 +
                'FROM ' +                                                       #13#10 +
              ...
              begin
                FkFieldsList3.Clear;
                if AnsiPos(',', q4.FieldByName('pk_fields').AsString) = 0 then
                  FkFieldsList3.Text := UpperCase(StringReplace(q4.FieldByName('pk_fields').AsString, '0=', '', [rfReplaceAll, rfIgnoreCase]))
                else
                begin
                  //TmpList.Sorted = False;
                  FkFieldsList3.Sorted := True;
                  FkFieldsList3.Text := UpperCase(StringReplace(q4.FieldByName('pk_fields').AsString, ',', #13#10, [rfReplaceAll, rfIgnoreCase]));

                  for I := 0 to FkFieldsList3.Count-1 do
                    TmpList.Append(FkFieldsList3.Values[FkFieldsList3.Names[I]]);
                  FkFieldsList3.Clear;
                  FkFieldsList3.Sorted := False;
                  FkFieldsList3.CommaText := TmpList.CommaText;

                  TmpList.Clear;
                end;
                // include pk Line åñëè ãëàâíîå ïîëå â HIS
            ...
            begin
              // ïîëó÷èì âñå íå ãëàâíûå FK cascade ïîëÿ â êðîññ-òàáëèöå
              q4.SQL.Text :=                                                    /// TODO: Prepare
                ...
                'WHERE  ' +                                                                  #13#10 +
                '  fc.relation_name = :rln ' +                                               #13#10 +
                '  AND REPLACE(TRIM(fc.list_fields), ''0='', '''') <> ''' + CrossLineTbls.Values[FkFieldsList3[N]] + ''' ' + #13#10 +
                '  AND fc.list_fields NOT LIKE ''%,%'' ';
              ...
              begin            LogEvent('5.2) 2.');
                ...
                begin
                  RefRelation := FkFieldsListLine.Values[FkFieldsListLine.Names[I]];
                  FkFieldsList.Clear;
                  FkFieldsList.Text := StringReplace(LineDocTbls.Values[FkFieldsListLine.Values[FkFieldsListLine.Names[I]]], '||', #13#10, [rfReplaceAll, rfIgnoreCase]);   // ãëàâíûå ïîëÿ òàáëèöû ref_relation_name
...
////////////
          if RealKolvo > 0 then // ïåðåîáðàáîòàåì â êîíöå øàãà
...
////////////
        ...
        // ñîõðàíèì îáðàáîòàííûå òàáëèöû ÷òîáû òîëüêî äëÿ íèõ îòêëþ÷àòü è óäàëÿòü
    ...
    LogEvent('DELETE FROM AC_RECORD...');
{
    ...
    // íîâûå äîêè è ñ òèïàìè óêàçàííûìè ïîëüçîâàòåëåì äîëæíû îñòàòüñÿ
...
{    q.SQL.Text :=
    ...
    CreateHIS(2); // inv_card.id íà êîòîðûå åñòü ññûëêè
    ProgressMsgEvent('', 100);
...
begin
  ...
  TmpList := TStringList.Create;
  FkFieldsList := TStringList.Create;
  RefFkFieldsList := TStringList.Create;
  ...
  try
    ...
    q2.SQL.Text :=                                                                                           //TESTME
      ...
      '  LIST(iseg.rdb$field_position || ''='' || TRIM(iseg.rdb$field_name))         AS Fields, ' +          #13#10 +
      '  LIST(ref_iseg.rdb$field_position || ''='' || TRIM(ref_iseg.rdb$field_name)) AS RefFields ' +        #13#10 +
      'FROM ' +                                                                                              #13#10 +
      '  rdb$relation_constraints c ' +                                                                      #13#10 +
      '  JOIN rdb$index_segments iseg ' +                                                                    #13#10 +
      '    ON iseg.rdb$index_name = c.rdb$index_name ' +                                                     #13#10 +
      '  JOIN RDB$REF_CONSTRAINTS refc ' +                                                                   #13#10 +
      '    ON refc.rdb$constraint_name = c.rdb$constraint_name ' +                                           #13#10 +
      '    JOIN RDB$RELATION_CONSTRAINTS c2 ' +                                                              #13#10 +
      '      ON c2.rdb$constraint_name = refc.rdb$const_name_uq ' +                                          #13#10 +
      '      JOIN rdb$index_segments ref_iseg ' +                                                            #13#10 +
      '        ON ref_iseg.rdb$index_name = c2.rdb$index_name ' +                                            #13#10 +
      'WHERE ' +                                                                                             #13#10 +
    ...
    while (not q2.EOF) and (not FDoStopProcessing) do
    begin
      FkFieldsList.Sorted := True;
      RefFkFieldsList.Sorted := True;
      FkFieldsList.CommaText := q2.FieldByName('Fields').AsString;
      RefFkFieldsList.CommaText := q2.FieldByName('RefFields').AsString;

      for I := 0 to FkFieldsList.Count-1 do
        TmpList.Append(FkFieldsList.Values[FkFieldsList.Names[I]]);
      FkFieldsList.Clear;
      FkFieldsList.Sorted := False;
      FkFieldsList.CommaText := TmpList.CommaText;
      TmpList.Clear;

      for I := 0 to RefFkFieldsList.Count-1 do
        TmpList.Append(RefFkFieldsList.Values[RefFkFieldsList.Names[I]]);
      RefFkFieldsList.Sorted := False;
      RefFkFieldsList.CommaText := TmpList.CommaText;
      TmpList.Clear;

      Str :=
        'ALTER TABLE inv_card ' +                                               #13#10 +
        '  ADD CONSTRAINT ' + q2.FieldByName('ConstraintName').AsString +       #13#10 +
        '  FOREIGN KEY (' + FkFieldsList.CommaText + ') ' +                     #13#10 +
        '  REFERENCES ' + q2.FieldByName('RefRelationName').AsString + '(' + RefFkFieldsList.CommaText + ') ';
    ...
    q2.SQL.Text :=                                                                        //TESTME
      ...
      '   c.rdb$constraint_name, ' +                                                      #13#10 +
      '   c.rdb$constraint_type, ' +                                                      #13#10 +
      '   LIST(i.rdb$field_position || ''='' || TRIM(i.rdb$field_name)) AS ListFields ' + #13#10 +
      ' FROM ' +                                                                          #13#10 +
      '   rdb$relation_constraints c ' +                                                  #13#10 +
    ...
    while (not q2.EOF) and (not FDoStopProcessing) do
    begin
      IndexFieldsList.Sorted := True;
      IndexFieldsList.CommaText := q2.FieldByName('ListFields').AsString;

      for I := 0 to IndexFieldsList.Count-1 do
        TmpList.Append(IndexFieldsList.Values[IndexFieldsList.Names[I]]);
      IndexFieldsList.Clear;
      IndexFieldsList.Sorted := False;
      IndexFieldsList.CommaText := TmpList.CommaText;
      TmpList.Clear;
...
begin
  ProgressMsgEvent('Îáúåäèíåíèå êàðòî÷åê...', 0);
  ...
  RestoreQueries := TStringList.Create;
  MergeQueries := TStringList.Create;
  Tr := TIBTransaction.Create(nil);
  q := TIBSQL.Create(nil);
  q2 := TIBSQL.Create(nil);
  try
    ...
    begin
      ...
      q.SQL.Text :=                                                             // êàðòî÷êè, îò êîòîðûõ õîòèì èçáàâèòüñÿ ïóòåì ïåðåïðèâÿçêè è óäàëåíèÿ
    ...
    q.SQL.Text :=                                                               // êàðòî÷êè ïî ýòèì äîêóìåíòàì áóäåì îáúåäèíÿòü
    ...
    q.SQL.Text :=                                                               // êàðòî÷êè ó÷àñòâóþùèå â äâèæåíèè âíå ïåðèîäà íå îáúåäèíÿåì
    ...
    ProgressMsgEvent('', 3*PROGRESS_STEP);    // 1%
    ...
    begin
      q.SQL.Add('  ' +
        '  DECLARE VARIABLE ' + StringReplace(q2.FieldByName('FieldsList').AsString, ';', ' = NULL;' +     #13#10 +
        '  DECLARE VARIABLE ', [rfReplaceAll, rfIgnoreCase]) + ' = NULL;' +                                #13#10 +
        '  DECLARE VARIABLE old_' + StringReplace(q2.FieldByName('FieldsList').AsString, ';', ' = NULL;' + #13#10 +
        '  DECLARE VARIABLE old_', [rfReplaceAll, rfIgnoreCase]) + ' = NULL; ');
    end;
    q.SQL.Add('  ' +
      'BEGIN  ' +                                                                                          #13#10 +
      '  FOR  ' +                                                                                          #13#10 +
    ...
    // ñîõðàíåíèå çàïðîñîâ äëÿ ïåðåïðèâÿçêè
    ...
    // îòêëþ÷åíèå ìåøàþùèõ ïåðåïðèâÿçêå êàðòî÷åê òðèããåðîâ
    ...
    // óäàëåíèå FKs, êðîìå èñïîëüçóåìûõ ïðè îáíîâëåíèè inv_card è inv_movement
    ...
    q2.SQL.Text :=
      ...
      'FROM ' +                                                                                              #13#10 +
      '  rdb$relation_constraints c ' +                                                                      #13#10 +
      '  JOIN RDB$REF_CONSTRAINTS refc ON c.rdb$constraint_name = refc.rdb$constraint_name ' +               #13#10 +
      '  JOIN RDB$RELATION_CONSTRAINTS c2 ON refc.rdb$const_name_uq = c2.rdb$constraint_name ' +             #13#10 +
      '  JOIN RDB$INDEX_SEGMENTS iseg ON iseg.rdb$index_name = c.rdb$index_name ' +                          #13#10 +
      '  JOIN RDB$INDEX_SEGMENTS ref_iseg ON ref_iseg.rdb$index_name = c2.rdb$index_name ' +                 #13#10 +
      'WHERE ' +                                                                                             #13#10 +
    ...
    begin
      FkFieldsList.Sorted := True;
      FkFieldsList.CommaText := q2.FieldByName('ListFields').AsString;
    ...
    // ïåðåïðèâÿçêà ññûëîê íà êàðòî÷êè

    for I:=0 to MergeQueries.Count-1 do
    begin
      q.SQL.Text := MergeQueries[I];
      if not FDoStopProcessing then
        ExecSqlLogEvent(q, 'MergeCards');
    end;
    ...
    // óäàëåíèå îñòàâøèõñÿ FKs inv_card
    q2.SQL.Text :=
      'SELECT ' +                                                                                            #13#10 +
      '  TRIM(c.rdb$constraint_name)                                                 AS ConstraintName, ' +  #13#10 +
      '  TRIM(c2.rdb$relation_name)                                                  AS RefRelationName, ' + #13#10 +
    ...
    // óäàëåíèå êàðòî÷åê, êîòîðûå áûëè îáúåäèíåíû
    ...
    // ïåðåñ÷åò ñêëàäñêîãî ñàëüäî INV_BALANCE
    CreateInvBalance;
    ...
    MsgBoxEvent('Îáúåäèíåíèå êàðòî÷åê çàâåðøåíî.');
  finally
    q.Free;
    q2.Free;
    Tr.Free;
...
//---------------------------------------------------------------------------
procedure TgsDBSqueeze.DeleteOldAcEntryBalance; // óäàëåíèå ñòàðîãî áóõãàëòåðñêîãî ñàëüäî
const
  IB_DATE_DELTA = 15018; // ðàçíèöà â äíÿõ ìåæäó "íóëåâûìè" äàòàìè Delphi è InterBase
var
  q: TIBSQL;
  Tr: TIBTransaction;
  CalculatedBalanceDate: TDateTime; // çíà÷åíèå ãåíåðàòîðà ñ äàòîé ïîñëåäíåãî ïîäñ÷åòà ENTRY BALANCE
begin
...
begin
  ...
  try
    ...
    q.SQL.Text :=
      'SELECT SUM(g_his_include(0, parent)) FROM gd_document WHERE g_his_has(0, id) = 1';
    ...
    q.SQL.Text :=
      ...
      '   AND id >= 147000000';                                                 /// ïðîâåðèòü ÷òî NULL äàåò his has = 0
    ExecSqlLogEvent(q, 'DeleteDocuments_DeleteHIS');
...
begin
  ...
  try
    ...
    // ïåðåíîñ äîêóìåíòîâ.  documentkey=masterdockey
    q2.SQL.Text :=
      'INSERT INTO GD_DOCUMENT ( ' +                                            #13#10 +
      '  id, ' +                                                                #13#10 +
      '  documenttypekey, ' +                                                   #13#10 +
...
var
  ...
  begin
    ...
    try
      q2.SQL.Text :=
        'SELECT ' +                                                             #13#10 +
        '  c.relation_name    AS RelationName, ' +                              #13#10 +
        '  c.constraint_name  AS ConstraintName, ' +                            #13#10 +
        '  c.constraint_type  AS ConstraintType, ' +                            #13#10 +
        '  c.list_fields      AS PosNameFields ' +                              #13#10 +
        'FROM dbs_pk_unique_constraints c ' +                                   #13#10 +
        'WHERE ' +                                                              #13#10 +
        '  c.relation_name NOT LIKE ''DBS_%'' ';
      ExecSqlLogEvent(q2, 'RestorePkUniqueConstraints');

      //TmpList.Sorted = False;
      while (not q2.EOF) do
      begin
        PkFieldsList.Sorted := True;
        PkFieldsList.CommaText := q2.FieldByName('PosNameFields').AsString;

        for I := 0 to PkFieldsList.Count-1 do
          TmpList.Append(PkFieldsList.Values[PkFieldsList.Names[I]]);
        PkFieldsList.Clear;
        PkFieldsList.Sorted := False;
        PkFieldsList.CommaText := TmpList.CommaText;
        TmpList.Clear;

        q.SQL.Text := 'ALTER TABLE ' + q2.FieldByName('RelationName').AsString +#13#10 +
          '  ADD CONSTRAINT ' + q2.FieldByName('ConstraintName').AsString +     #13#10 +
          '  ' +  q2.FieldByName('ConstraintType').AsString + ' (' + PkFieldsList.CommaText + ') ';
  ...
  begin
    ...
    try
      q2.SQL.Text :=
        ...
        '  c.relation_name     AS RelationName, ' +                             #13#10 +
        '  c.constraint_name   AS ConstraintName, ' +                           #13#10 +
        '  c.list_fields       AS Fields, ' +                                   #13#10 +
        '  c.ref_relation_name AS RefRelationName, ' +                          #13#10 +
        '  c.list_ref_fields   AS RefFields, ' +                                #13#10 +
        '  TRIM(c.update_rule) AS UpdateRule,  ' +                              #13#10 +
        '  TRIM(c.delete_rule) AS DeleteRule ' +                                #13#10 +
        'FROM ' +                                                               #13#10 +
        '  dbs_fk_constraints c ' +                                             #13#10 +
        'WHERE ' +                                                              #13#10 +
        '  c.constraint_name NOT LIKE ''DBS_%'' ';
      ExecSqlLogEvent(q2, 'RestoreFKConstraints');
      ...
      begin
        ...
        q.SQL.Text :=
          ...
          '  REFERENCES ' + q2.FieldByName('RefRelationName').AsString + '(' + RefFkFieldsList.CommaText + ') ';
        if UpperCase(q2.FieldByName('UpdateRule').AsString) <> 'RESTRICT' then
          q.SQL.Add(' ON UPDATE ' + q2.FieldByName('UpdateRule').AsString);
        if UpperCase(q2.FieldByName('DeleteRule').AsString) <> 'RESTRICT' then
          q.SQL.Add(' ON DELETE ' + q2.FieldByName('DeleteRule').AsString);
...
begin
  ...
  try
    ...
    q2.SQL.Text :=
      ...
      '  AND rdb$trigger_inactive = 0';
    if not FDoStopProcessing then
      ExecSqlLogEvent(q2, 'CreateInvBalance');
...
begin
  ...
  try
    ...
    try
      ...
      on E: Exception do
      begin
        ErrorEvent('Îøèáêà ïðè óäàëåíèè òàáëèöû: ' + E.Message);
      end;
    end;
  finally
    q.Free;
...
const
  Ms = 1 / (24 * 60 * 60 * 1000); // Çíà÷åíèå 1 ìèëëèñåêóíäû â ôîðìàòå TDateTime
var
  I: Integer;
...
//---------------------------------------------------------------------------
procedure TgsDBSqueeze.MsgBoxEvent(const AMsg: String);
var
  PI: TgdProgressInfo;
begin
  if Assigned(FOnProgressWatch) then
