  ...
  try
    ...
    try
      ...
      SQL.SQL.Text :=
        ...
        'AS '#13#10 +
        'BEGIN '#13#10 +
        '  IF (UPPER(OLD.name) IN (''ТАРА'', ''СТЕКЛОПОСУДА'', ''ДРАГМЕТАЛЛЫ'')) THEN '#13#10 +
        '    EXCEPTION gd_e_cannotchange_goodgroup  ''Нельзя удалить группу '' || OLD.Name; '#13#10 +
        'END ';
      SQL.ExecQuery;
      SQL.Close;
      ...
      SQL.SQL.Text :=
        ...
        'AS '#13#10 +
        'BEGIN '#13#10 +
        '  IF ((UPPER(NEW.name) IN (''ТАРА'', ''СТЕКЛОПОСУДА'', ''ДРАГМЕТАЛЛЫ'')) '#13#10 +
        '    OR (UPPER(OLD.name) IN (''ТАРА'', ''СТЕКЛОПОСУДА'', ''ДРАГМЕТАЛЛЫ''))) THEN '#13#10 +
        '  BEGIN '#13#10 +
        '    IF (NEW.name <> OLD.name OR NEW.parent IS DISTINCT FROM OLD.parent) THEN '#13#10 +
        '      EXCEPTION gd_e_cannotchange_goodgroup  ''Нельзя изменить группу '' || NEW.Name; '#13#10 +
        '  END '#13#10 +
        'END ';
      SQL.ExecQuery;
      SQL.Close;
