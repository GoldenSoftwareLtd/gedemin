...
const
  ...
  Tables: array [0..TableCount - 1] of TTable = (
    ...
    (Name: 'RPL$DBSTATE'; Body:
      ...
      ' PREPARED             DRPLINTEGER_NOT_NULL CHECK (PREPARED IN (0, 1))); '#13#10 +
...
const
  cTriggerBody =  '   INSERT INTO RPL$LOG(RELATIONKEY, REPLTYPE, OLDKEY, NEWKEY) ' +
    '     VALUES(%d, %s, %s, %s);';
...
const
  ...
  ADD_DBSTATE_FKFIELD =
    'ALTER TABLE RPL$DBSTATE ' +
    'ADD FKETALON DRPLBLOB';
...
const
  ...
  cFileNotFound = 'Файл не найден';
  ConnectionError = 'Ошибка соединения с базой данных';
  ReplFileFilter = 'Файлы ссылок|*.rfk|Текстовые файлы|*.txt|All files|*.*';
  ...
  MSG_ADD_FK_INFO = 'Добавление информации о внешних ссылках';
