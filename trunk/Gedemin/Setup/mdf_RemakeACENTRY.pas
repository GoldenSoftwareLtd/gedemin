  ...
  try
    ...
    try
      ...
      StoredProcedure.Description :=
        ...
        '    dontinmove integer /* не включать внутренние проводки */) ' + #13#10 +
        'returns ( ' + #13#10 +
...
begin
  ...
  try
    ...
    try
      ...
      Log('Добавление поля companykey в AC_ENTRY');
      ...
      Log('Заполнение добавленных полей');
      q.SQL.Text :=
      ...
      Log(Format('Добавление foreign key %s в таблицу %s', [AlterConstraints.ConstraintName,
      ...
      begin
        Log(Format('Добавление индекса %s в таблицу %s', [Index.IndexName,
          Index.RelationName]));
        try
          AddIndex(Index, IBDB);
        except
      ...
      Log(Format('Добавление триггера %s ', [AlterTrigger.TriggerName]));
      try
