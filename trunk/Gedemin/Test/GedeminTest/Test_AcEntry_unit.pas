...
type
  ...
  private
    // провер€ет правильность установки флага issimple
    // дл€ позиций сложных и простых проводок
    procedure TestIsSimpleField(q: TIBSQL);

    // метод провер€ет соответствие сумм в шапке проводки и
    // позици€х, равенство сумм в шапке дл€
    // корректных проводок, а также правильность заполнени€
    // дебетовых сумм дл€ кредлитов и наоборот
    procedure TestEqualOfDebitAndCreditSum(q: TIBSQL);

    // провер€ет наличие в базе проводок, помеченных как incorrect
    procedure TestIncorrectRecords(q: TIBSQL);

    // провер€ет соответствие полей в шапке и позиции
    // проводки
    procedure TestEntryConsistency(q: TIBSQL);

    // провер€ет отсутствие флагов разблокировки изменени€ записи
    procedure TestUnlockFlags(q: TIBSQL);

    // провер€ет количество дебетов и кредитов в сложной проводке
    procedure TestEntryCount(q: TIBSQL);

    // все вышеперечисленные проверки, вместе вз€тые
...
begin
  ...
  q.ExecQuery;
  Check(q.EOF, '—ложные проводки с множественными дебетами и кредитами');
end;
...
begin
  ...
  q.ExecQuery;
  Check(q.EOF, 'ѕроверка дебетовой и кредитовой части в шапке проводки');
  ...
  q.ExecQuery;
  Check(q.EOF, 'ѕроверка соответстви€ сумм в шапке и позици€х проводки');
  ...
  q.ExecQuery;
  Check(q.Fields[0].AsCurrency = 0, 'Ќенулевые суммы по дебету дл€ кредитовых частей');
  ...
  q.ExecQuery;
  Check(q.Fields[0].AsCurrency = 0, 'Ќенулевые суммы по кредиту дл€ дебетовых частей');
end;
...
begin
  ...
  q.ExecQuery;
  Check(q.EOF, '«аписи в AC_RECORD с incorrect = 1');
end;
...
begin
  ...
  q.ExecQuery;
  Check(q.EOF, 'ѕроверка пол€ issimple');
end;
...
begin
  ...
  // поле incorrect нельз€ изменить вручную
  ...
  // провер€ем очистку переменной AC_RECORD_INCORRECT
  // без закрыти€ транзакции
  FQ.Close;
  FQ.SQL.Text :=
    'SELECT RDB$GET_CONTEXT(''USER_TRANSACTION'', ''AC_RECORD_INCORRECT'') FROM rdb$database';
  FQ.ExecQuery;
