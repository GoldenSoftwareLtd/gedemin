...
{++
  ...
  Revisions history
    ...
    beta3    14-02-99  Dennis  Decimal Separator bug fixed.
                               Было произведена попукта изменения сообщения 
...
uses
  ...
  StdCtrls, Buttons, ExtCtrls, DB, DBCtrls;
...
procedure TxCalculatorEdit.SetDecDigits(ADecDigits: Word);
begin
...
//  Exit;
  ...
  if not (csDesigning in ComponentState) then
    case Key of
      VK_ESCAPE, VK_MENU: if FCalculator.Visible then
      begin
        FCalculator.Hide;
        ExpressionAction := eaNone;
      end;
...
//  Exit;
  ...
  begin
    ...
    begin
      if not FCalculator.Visible and (Key in ['-', '+', '*', '/']) then
      ...
      if (AnsiCompareText(Key, 'C') = 0) and FCalculator.Visible then
      begin
        Value := 0;
        ExpressionAction := eaEqual;
        OldValue := 0;
      ...
      end else if ((Ord(Key) = VK_RETURN) or (Key = '=')) and FCalculator.Visible then
      begin // Если нажата кнопка Enter или знак "="
        ...
        OldValue := 0;
        OldValueEntered := False;
      end else if (Key in ['+', '-', '*', '/']) and FCalculator.Visible then
      begin // Если выбран один из вышеперечисленных знаков
        if OldValueEntered and not NeedToChangeValue then Value := CountValue;
        ...
        begin
          ...
          NeedToChangeValue := True;
        end else if not NeedToChangeValue then
        begin
          OldValue := 0;
          OldValueEntered := False;
        end;
      ...
      begin // Если нажата кнопка BACKSPACE
        if ExpressionAction = eaNone then Key := #0;
      end else if NeedToChangeValue and FCalculator.Visible then
      begin // Если нужно подставить новое число
        NeedToChangeValue := False;
        if ExpressionAction = eaNone then ExpressionAction := eaEqual;
    ...
    if (Key <> #0) and (Text = '0') and (Key <> DecimalSeparator) then Text := '';
  ...
  if FCalculator.Visible then SelStart := Length(Text) + 1;
end;
...
//  ControlStyle := ControlStyle - [csSetCaption] + [csReplicatable]; // Change

  // Создаем кнопку и указываем ее параметры
  ...
  if not (csDesigning in ComponentState) then
  begin
    // Создаем калькулятор и указываем его параметры
    FCalculator := TxCalculator.Create(AnOwner);
...
begin
  if FDataLink.Field <> nil then Value := FDataLink.Field.AsFloat;
end;
